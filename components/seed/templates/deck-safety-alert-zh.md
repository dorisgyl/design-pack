---
slug: packs/design-pack/templates/deck-safety-alert-zh
type: template
lang: zh
category: slides
title: "安全 / 风险红色 Deck"
title_en: "NevoFlux Safety Alert Deck"
description: "红琥珀警示色 + hazard 条纹 + L1/L2/L3 分级卡片的高对比 deck,用于 NevoFlux 安全、风险、事故复盘与 policy-as-code 简报。"
tags: [safety, security, policy, incident, 模板]
...
面向安全 / 风险 / 事故复盘 / red team / policy-as-code 的警示型 deck。意图是用高对比的红 / 琥珀警示色,在温暖白底上传达风险、紧迫感与明确的处置方案。

### 布局
- 顶部(粗)与底部(细、半透明)的 45° 红黑 hazard 条纹。
- 红色删除线否定标题:用一道斜切的红条划掉错误的假设。
- L1/L2/L3 绿 / 琥珀 / 红 分级卡片,每张顶部加一条彩色边框表示严重度。
- 圆点状态 alert box:左上角有一个悬浮圆点徽标,按严重度着色(红 / 琥珀 / 绿)。
- policy-yaml 代码块:红色左边框 + 对 bad / 禁止关键词高亮。
- 红绿 checklist(通过项是绿色对勾框,未完成项是琥珀色感叹号)+ 事故堆叠柱状图。

### 设计细节
- 配色要克制:危险用红 `#e0314a`,警告用琥珀 `#d97706`,安全用绿 `#067647`,底色用温暖纸白 `#fffaf7`。
- chrome 行里的 alert-tag 胶囊会换色(红 / 琥珀 / 绿),并切换前置图标(警告 vs 对勾),以匹配当页基调。
- 删除线标题只用一两次以获得最大冲击力 —— 这是本 deck 的标志性手法。
- 堆叠柱状图把 L1(绿)放底部、L2(琥珀)放中间、L3(红)放顶部,让视线先读到最坏情况的严重度。
...
<div class="deck">
  ...
  <section class="slide is-active">
    ...
    <div class="ts-chrome"><span class="ts-alert-tag">agent 安全 · 高优先级</span><span class="ts-page">01 / 08</span></div>
    <div class="ts-kicker">2026 年最重要的一条判断</div>
    <h1 class="ts-h1">别再追问<br><span class="strike">NevoFlux agent 会不会干活</span><br>开始问:<span class="red">它出事谁负责</span></h1>
    <p class="ts-sub">NevoFlux agent 出错的代价,不再是一次 bad response 这么简单 —— 它可能在一次 Canvas 运行里写 300 份工单、提 80 个 PR、发 5000 封邮件。</p>
    <div class="ts-alert-box">
      <h3>风险已经规模化</h3>
      <p>「做错」成本 × N;「做对」收益 × N。<br>这就是为什么 <b>测试、验收、安全、风控</b> 会变成 NevoFlux 未来 3 年最贵的能力。</p>
    </div>
  ...
  <section class="slide">
    ...
    <div style="margin:auto 0">
      ...
      <p class="ts-sub" style="font-size:28px">不是所有 agent 行为都同等危险。<br>先把「可撤销」和「不可撤销」分开,再谈流程。</p>
    </div>
    <div class="ts-stripe-b"></div>
  ...
  <section class="slide">
    ...
    <div class="ts-grid-3">
      <div class="ts-card" style="border-top:4px solid var(--ts-green)"><div class="lbl">L1 · 绿色</div><h4>可撤销</h4><p>写 draft、生成 Canvas 草稿、读 GBrain 文档。<br>错了 Ctrl+Z,零代价。<br><b style="color:var(--ts-green)">策略:放开跑</b></p></div>
      <div class="ts-card" style="border-top:4px solid var(--ts-amber)"><div class="lbl">L2 · 琥珀</div><h4>半可撤销</h4><p>发 draft 邮件、提 PR、改 staging GBrain。<br>错了要道歉 / 回滚。<br><b style="color:var(--ts-amber)">策略:人工复核</b></p></div>
      <div class="ts-card" style="border-top:4px solid var(--ts-red)"><div class="lbl">L3 · 红色</div><h4>不可撤销</h4><p>发真实邮件、付款、删 pack、删 prod GBrain 数据。<br>错了就真错了。<br><b style="color:var(--ts-red)">策略:硬卡 + 双人审</b></p></div>
    </div>
...
<span class="bad">forbidden_always</span>:
  ...
  <section class="slide">
    ...
    <h2 class="ts-h2">我们 Q1 的 <span class="red">12 起 agent 事故</span></h2>
    ...
    <svg viewBox="0 0 1040 360" style="width:100%;max-width:1040px;margin-top:18px" xmlns="http://www.w3.org/2000/svg">
      <g font-family="Inter,sans-serif" font-size="14" fill="#4a4955">
        ...
        <g transform="translate(720,60)">
          ...
          <text x="0" y="100" font-size="13" fill="#8a8892">全部被 nevoflux-policy 在 runtime 拦下,</text>
  ...
  <section class="slide">
    ...
    <div class="ts-checklist">
      ...
      <div class="ts-check ok"><div class="box">✓</div><div class="txt">上下游失败时,它会不会开始乱改其他 GBrain 资源?</div></div>
      <div class="ts-check"><div class="box">!</div><div class="txt">并发 100 个 agent 一起跑会不会把浏览器拖死锁?</div></div>
  ...
  <section class="slide">
    ...
    <div class="ts-grid-3">
      ...
      <div class="ts-card"><div class="lbl">2 · 写 policy</div><h4>policy.yaml<br>接 runtime</h4><p>不要信 prompt 里的 "be careful",要信 SDK 执行层的硬卡。</p></div>
  ...
  <section class="slide">
    ...
    <div style="margin:auto 0">
      ...
      <p class="ts-sub" style="font-size:24px">policy.yaml 模板、红队 prompt 清单、事故复盘模板 —— 都在 NevoFlux 安全 pack 里。</p>
    </div>
    <div class="ts-stripe-b"></div>
...
按顺序填这些区块,每个 `<section class="slide">` 是一张 16:9 幻灯片:
- 第 1 页(封面):alert-tag、kicker、删除线否定标题、副标题,以及陈述规模化风险的标题 alert box。
- 第 2 页(章节分隔):章节 kicker + 超大标题 + 一句话框定第一章。
- 第 3 页(风险分级):三张 L1/L2/L3 卡片(绿 / 琥珀 / 红)+ 一个警告「不要自我升级」的琥珀 alert box。
- 第 4 页(policy as code):`ts-codebox` 的 YAML 块 —— 保留 `cm` / `kw` / `st` / `bad` 这几个 span 做语法高亮;`bad` 标记禁止命令。
- 第 5 页(事故图表):内联 SVG 堆叠柱状图(L1 绿在底、L2 琥珀、L3 红在顶)+ 图例;改 rect 高度和计数标签即可。
- 第 6 页(checklist):红绿清单 —— 给 `.ts-check` 加 `.ok` 是通过的绿色对勾,不加则是未完成的琥珀项。
- 第 7 页(CTA):三张行动卡片 + 一个绿色收尾 alert box。
- 第 8 页(致谢):超大标题收尾页 + 一句资源召唤。
chrome 行里的 alert-tag 胶囊可加 `amber` 或 `green` 修饰类以匹配当页基调;保持 `ts-page` 计数器和 `ts-footer` 标签同步。
