---
slug: packs/design-pack/templates/deck-graphify-dark
type: template
lang: en
category: slides
title: "Graphify Dark Deck"
title_zh: "暗底图谱 Deck"
description: "Dark-gradient launch deck with floating orbs, an SVG force-directed knowledge graph, and JetBrains Mono code slides."
tags: [graph, dev tool, ai, cli, template]
...
A dark, AI-native presentation deck built for knowledge-graph and developer-tool launches. The mood is late-night gradient plus glassmorphism.

Layout:
- **Cover**: a `#06060c → #0e1020` gradient background with drifting blurred orbs and a live SVG force-directed graph rendered behind the title.
- **Section dividers**: oversized headings, with one keyword carrying an animated rainbow or warm gradient text fill.
- **Code / CLI slides**: JetBrains Mono with syntax highlighting (comment / keyword / string / function colors) inside a dark code box, plus a glowing single-line command.
- **Glassmorphism cards**: translucent frosted panels (`backdrop-filter: blur`) in warm, blue, and green tints for feature grids and metrics.

Design details:
- Color tokens are scoped under `.tpl-graphify-dark-graph`; warm sand, sky blue, mint green, rose, and purple accents sit on near-black backgrounds.
- Each slide carries a slide number (`01 / 08`) top-right and three ambient orbs that animate with a slow drift keyframe.
- Big numeric metrics use a 120px weight-900 figure with a gradient text fill.
- Aspect ratio is 16:9; the static-preview fallback keeps every slide visible when the runtime is absent.
...
<title>NevoFlux GBrain Deck</title>
...
<style>/* graphify-dark-graph — dark glass + force-directed knowledge graph */
...
<div class="deck">
  ...
  <section class="slide is-active">
    ...
    <div style="margin-top:auto">
      <p class="gd-eyebrow">Tech Sharing · No Fluff</p>
      <h1 class="gd-h1" style="font-size:88px"><span class="gd-rainbow">Build your GBrain<br>knowledge graph in NevoFlux</span></h1>
      <p class="gd-lede" style="margin-top:20px">One command · Fully multimodal · Honest audit trail — <span class="gd-accent">turn any folder into a navigable knowledge network.</span></p>
      <p class="gd-eyebrow" style="margin-top:26px">↑ This background is a real graph rendered from NevoFlux GBrain</p>
    </div>
  </section>
  ...
  <section class="slide">
    ...
    <div style="margin:auto 0">
      ...
      <h1 class="gd-h1" style="font-size:120px">Why a <span class="gd-grad">Graph</span>?</h1>
      <p class="gd-lede">folder → tree → graph, the next step in how we navigate knowledge</p>
    </div>
  </section>
  ...
  <section class="slide">
    ...
    <h2 class="gd-h2">One browser, <span class="gd-grad">four jobs</span></h2>
    <div class="gd-grid-4">
      <div class="gd-glass gd-glass-warm"><div style="font-size:30px">📂</div><h4 style="margin:10px 0 6px">Folder Ingest</h4><p class="gd-dim" style="font-size:13px;line-height:1.55">Recursively scan any path — md / pdf / code / images</p></div>
      <div class="gd-glass gd-glass-blue"><div style="font-size:30px">🧠</div><h4 style="margin:10px 0 6px">Entity Extract</h4><p class="gd-dim" style="font-size:13px;line-height:1.55">The agent pulls concepts, people, events, relations</p></div>
      <div class="gd-glass gd-glass-green"><div style="font-size:30px">🕸️</div><h4 style="margin:10px 0 6px">Force Graph</h4><p class="gd-dim" style="font-size:13px;line-height:1.55">A Canvas app you click to jump back to the source</p></div>
      <div class="gd-glass"><div style="font-size:30px">🔍</div><h4 style="margin:10px 0 6px">Audit Trail</h4><p class="gd-dim" style="font-size:13px;line-height:1.55">Every edge traces back to a source span</p></div>
    </div>
    <div class="gd-glass gd-glass-warm" style="margin-top:24px"><p style="font-size:18px;line-height:1.6">GBrain is not "yet another RAG" — it <span class="gd-accent">draws your retrieval results so you can see, at a glance, what the information looks like</span>.</p></div>
  </section>
  ...
  <section class="slide">
    ...
    <h2 class="gd-h2">From zero to graph, <span class="gd-grad">about 90 seconds</span></h2>
...
<span class="kw">render</span>:
  ...
  audit: <span class="fn">true</span>     <span class="cm"># every edge carries a source span</span></pre>
  </section>
  ...
  <section class="slide">
    ...
    <h2 class="gd-h2">Without GBrain vs with GBrain</h2>
    <div style="max-width:900px;margin-top:30px">
      <div style="display:flex;align-items:center;gap:16px;margin-bottom:20px">
        <div style="width:110px;text-align:right;font-weight:700;color:var(--gd-danger)">Without<br>GBrain</div>
        <div style="flex:1;position:relative;height:70px;background:rgba(224,112,112,.06);border:1px solid rgba(224,112,112,.2);border-radius:16px">
          ...
          <div style="position:absolute;left:72px;top:50%;transform:translateY(-50%);color:var(--gd-danger);font-size:14px">Re-feed context… re-organize… forgotten again…</div>
        </div>
      </div>
      <div style="display:flex;align-items:center;gap:16px">
        <div style="width:110px;text-align:right;font-weight:700;color:var(--gd-green)">With<br>GBrain</div>
        <div style="flex:1;position:relative;height:70px;background:rgba(126,211,164,.06);border:1px solid rgba(126,211,164,.25);border-radius:16px">
          ...
          <div style="position:absolute;right:72px;top:50%;transform:translateY(-50%);color:var(--gd-green);font-size:14px">Agent finds it → confirms → gets to work!</div>
        </div>
      </div>
    </div>
    <div class="gd-grid-3" style="margin-top:36px">
      <div class="gd-glass gd-glass-warm"><div class="gd-big gd-grad">5×</div><p class="gd-dim" style="margin-top:6px">Faster work</p></div>
      <div class="gd-glass gd-glass-green"><div class="gd-big gd-grad">-80%</div><p class="gd-dim" style="margin-top:6px">Repeated context</p></div>
      <div class="gd-glass gd-glass-blue"><div class="gd-big gd-grad">∞</div><p class="gd-dim" style="margin-top:6px">Persistent memory</p></div>
    </div>
  </section>
  ...
  <section class="slide">
    ...
    <h2 class="gd-h2">End to end in <span class="gd-grad">4 steps</span></h2>
    <div style="display:flex;align-items:center;justify-content:center;gap:8px;margin-top:36px">
      <div class="gd-glass" style="flex:1;text-align:center"><div style="font-size:34px">📂</div><div style="font-weight:600;margin-top:8px">Scan</div><div class="gd-dim" style="font-size:13px">Read files recursively</div></div>
      <div style="color:var(--gd-text3);font-size:24px">→</div>
      <div class="gd-glass gd-glass-blue" style="flex:1;text-align:center"><div style="font-size:34px">🔬</div><div style="font-weight:600;margin-top:8px">Extract</div><div class="gd-dim" style="font-size:13px">Agent pulls entities</div></div>
      <div style="color:var(--gd-text3);font-size:24px">→</div>
      <div class="gd-glass gd-glass-green" style="flex:1;text-align:center"><div style="font-size:34px">🕸️</div><div style="font-weight:600;margin-top:8px">Build</div><div class="gd-dim" style="font-size:13px">Graph + dedupe</div></div>
      <div style="color:var(--gd-text3);font-size:24px">→</div>
      <div class="gd-glass gd-glass-warm" style="flex:1;text-align:center"><div style="font-size:34px">🎨</div><div style="font-weight:600;margin-top:8px">Render</div><div class="gd-dim" style="font-size:13px">Interactive Canvas app</div></div>
    </div>
    <div class="gd-glass" style="margin-top:32px"><p style="font-size:16px;line-height:1.6;color:var(--gd-text2)">Every step keeps an audit log: you always know why a node exists and which line of which file it came from.</p></div>
  </section>
  ...
  <section class="slide">
    ...
    <h2 class="gd-h1" style="font-size:80px">Graph <span class="gd-grad">your folders</span></h2>
  ...
  <section class="slide">
    ...
    <div style="margin:auto 0;text-align:center">
      ...
      <p class="gd-lede" style="margin:28px auto 0">nevoflux.dev/gbrain · packs, Canvas apps, and design skills welcome</p>
    </div>
  </section>
...
- **Slide 1 (Cover)**: eyebrow kicker, rainbow hero title, lede, and a credit line. The SVG behind it is a placeholder graph — swap node/edge coordinates and colors to match your data.
- **Slide 2 (Section divider)**: `Part` label plus an oversized heading with one gradient keyword.
- **Slide 3 (Feature grid)**: four `gd-glass` cards (icon · `h4` title · one-line description), then a wide highlight card for the positioning line.
- **Slide 4 (Code)**: one glowing `gd-cmd` command and a `gd-codebox` config block using `.cm` / `.kw` / `.st` / `.fn` spans for highlighting.
- **Slide 5 (Race chart)**: two horizontal lanes (before / after) plus three big-number metric cards.
- **Slide 6 (Pipeline)**: four stage cards joined by arrows, then a note card.
- **Slide 7 (CTA)**: install/run commands and a row of `gd-tag` hashtags.
- **Slide 8 (Thanks)**: rainbow sign-off plus a link/handle line.
- Keep slide numbers (`gd-snum`) and the `01 / 08` count in sync if you add or remove slides.
