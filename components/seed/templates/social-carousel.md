---
slug: packs/design-pack/templates/social-carousel
type: template
lang: en
category: card
title: "NevoFlux Social Carousel"
title_zh: "社交媒体三联"
description: "Three 1080×1080 square cards whose headlines chain into one sentence, each with a brand mark and 1/3 numbering."
tags: [instagram, linkedin, thread, carousel, 三联, template]
...
Layout
- Three 1080×1080 square cards. The display headline runs across all three so they read as one continuous line.
- Card 1: opening of the headline + brand mark + index `1/3`.
- Card 2: middle of the headline + the visual focal point + index `2/3`.
- Card 3: end of the headline + CTA + loop icon + index `3/3`.

Design details
- Use one unified palette across the set, shifting gradually from card to card so the three feel like one sequence.
- The three headlines, when joined, form one complete sentence.
- Keep type minimal and large; let the CSS-only background art carry the mood. Each card stands on its own or runs as a three-part series on Instagram, LinkedIn, or X.
...
<head>
  ...
  <title>Browse it. Brain it. Loop it. — NevoFlux carousel</title>
...
<body>
  <div class="stage" data-od-id="stage">
    <div class="stage-head">
      <div>
        <h1>Browse it<span class="dot">.</span> Brain <em>it</em><span class="dot">.</span></h1>
        <p class="lede">1080×1080 · NevoFlux browser carousel · minimal type. Drop into Instagram, LinkedIn, or X — each post stands on its own or runs as a three-part series.</p>
      </div>
    ...
    <div class="row" data-od-id="cards">
      ...
      <article class="card c1" data-od-id="card-1">
        ...
        <div class="top">
          ...
          <div class="index">01 · BROWSE</div>
        </div>
        <div class="lockup"><h2>browse it<em>.</em></h2></div>
        <div class="footer">
          <div class="caption">Open any tab — the browser is watching for you.</div>
      ...
      <article class="card c2" data-od-id="card-2">
        ...
        <div class="top">
          <div class="brand"><span class="dot"></span><span class="name">NevoFlux</span></div>
          <div class="index">02 · BRAIN IT</div>
        </div>
        <div class="lockup"><h2><span class="accent">brain it</span><br/><em>into GBrain.</em></h2></div>
        <div class="footer">
          <div class="caption">One clip — saved to your knowledge base.</div>
          <div class="loop">1× LOOP</div>
        </div>
      </article>
      ...
      <article class="card c3" data-od-id="card-3">
        ...
        <div class="top">
          <div class="brand"><span class="dot"></span><span class="name">NevoFlux</span></div>
          <div class="index">03 · LOOP IT</div>
        </div>
        <div class="lockup"><h2>loop it<br/><em>in Canvas.</em></h2></div>
        <div class="footer">
          <div class="caption">Turn what you saved into a Canvas app.</div>
          <div class="loop">1× LOOP</div>
        </div>
      </article>
...
- `stage-head h1`: the kicker headline for the contact sheet view; keep two short beats joined by `.dot` periods.
- `stage-head .lede`: one uppercase mono line describing format and channels. `badge`: the series range, e.g. `SERIES · 01 → 03`.
- Each `.card` is one 1080×1080 frame. Fill three slots:
  - `.brand .name`: the brand mark shown in the pill (here "NevoFlux").
  - `.index`: the frame number plus a one-word verb, e.g. `01 · BROWSE`.
  - `.lockup h2`: the headline fragment. Wrap the accented word in `<em>` or `<span class="accent">`; the three fragments must join into one sentence ("Browse it. Brain it into GBrain. Loop it in Canvas.").
  - `.footer .caption`: a short uppercase mono caption describing the frame's visual.
  - `.loop`: the loop badge; leave as `1× LOOP`.
- The `.figure`, `.trees`, and `.ridge` shapes are pure CSS background art — keep them to carry the mood and the card-to-card palette shift.
