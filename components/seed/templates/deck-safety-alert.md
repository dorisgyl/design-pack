---
slug: packs/design-pack/templates/deck-safety-alert
type: template
lang: en
category: slides
title: "NevoFlux Safety Alert Deck"
title_zh: "安全 / 风险红色 Deck"
description: "High-contrast red/amber hazard-striped slide deck for safety, risk, incident reviews, and policy-as-code briefings about NevoFlux."
tags: [safety, security, policy, incident, template]
sample_image: packs/design-pack/assets/templates/deck-safety-alert.svg
source: html-anything/deck-safety-alert
---

## Design guidance

A safety alert deck for risk, incident postmortems, red-team findings, and policy-as-code briefings. The intent is to communicate hazard, urgency, and clear remediation with a high-contrast red/amber alert aesthetic on a warm white background.

### Layout
- 45-degree red/black hazard stripes at the top (thick) and bottom (thinner, faded).
- Red negation headlines with a skewed strike-through bar that crosses out the wrong assumption.
- L1/L2/L3 tier cards in green / amber / red, each topped with a colored border to signal severity.
- Alert boxes with a floating dot badge in the top-left corner, tinted by severity (red / amber / green).
- A policy-YAML code block with a red left border and highlighted "bad" / forbidden keywords.
- A red/green checklist (passing items get a green check box, open items get an amber exclamation) plus a stacked incident bar chart.

### Detail notes
- Keep the palette disciplined: red `#e0314a` for danger, amber `#d97706` for caution, green `#067647` for safe, on warm paper `#fffaf7`.
- The alert tag pill in the chrome row flips color (red / amber / green) and swaps its leading glyph (warning vs check) to match the slide's tone.
- Use the strike headline only once or twice for maximum impact - it is the deck's signature move.
- The stacked bar chart stacks L1 (green) at the bottom, L2 (amber) in the middle, L3 (red) on top, so the eye reads worst-case severity first.
...
<title>NevoFlux Safety Alert</title>
...
<div class="deck">
  ...
  <section class="slide is-active">
    ...
    <div class="ts-chrome"><span class="ts-alert-tag">agent safety · high priority</span><span class="ts-page">01 / 08</span></div>
    <div class="ts-kicker">The call that matters most in 2026</div>
    <h1 class="ts-h1">Stop asking<br><span class="strike">can the NevoFlux agent do work</span><br>Start asking: <span class="red">who owns it when it breaks</span></h1>
    <p class="ts-sub">When a NevoFlux agent gets it wrong, the cost is no longer one bad response — it can file 300 tickets, open 80 PRs, and email 5,000 contacts in a single Canvas run.</p>
    <div class="ts-alert-box">
      <h3>The risk now scales</h3>
      <p>"Wrong" cost × N; "right" payoff × N.<br>That is why <b>testing, acceptance, safety, and risk control</b> become NevoFlux's most expensive capabilities over the next 3 years.</p>
    </div>
    <div class="ts-stripe-b"></div>
    <div class="ts-footer"><span>NEVOFLUX SAFETY BRIEF · PLATFORM TEAM · 2026.06</span><span>01 / 08</span></div>
  </section>

  <!-- 2. SECTION -->
  <section class="slide">
    <div class="ts-stripe"></div>
    <div class="ts-chrome"><span class="ts-alert-tag amber">section · risk tiers</span><span class="ts-page">02 / 08</span></div>
    <div style="margin:auto 0">
      <div class="ts-kicker">Chapter One</div>
      <h1 class="ts-h1" style="font-size:130px">Tier <span class="red">first</span></h1>
      <p class="ts-sub" style="font-size:28px">Not every agent action is equally dangerous.<br>Separate "reversible" from "irreversible" before you talk about process.</p>
    </div>
    <div class="ts-stripe-b"></div>
  ...
  <section class="slide">
    <div class="ts-stripe"></div>
    <div class="ts-chrome"><span class="ts-alert-tag">risk tiers · 3 levels</span><span class="ts-page">03 / 08</span></div>
    <h2 class="ts-h2">Three tiers, three responses</h2>
    <div class="ts-grid-3">
      <div class="ts-card" style="border-top:4px solid var(--ts-green)"><div class="lbl">L1 · green</div><h4>Reversible</h4><p>Write drafts, generate Canvas mockups, read GBrain docs.<br>Wrong? Ctrl+Z, zero cost.<br><b style="color:var(--ts-green)">Policy: let it run</b></p></div>
      <div class="ts-card" style="border-top:4px solid var(--ts-amber)"><div class="lbl">L2 · amber</div><h4>Semi-reversible</h4><p>Send draft emails, open a PR, write to staging GBrain.<br>Wrong? Apologize / roll back.<br><b style="color:var(--ts-amber)">Policy: human review</b></p></div>
      <div class="ts-card" style="border-top:4px solid var(--ts-red)"><div class="lbl">L3 · red</div><h4>Irreversible</h4><p>Send real emails, make payments, drop a pack, delete prod GBrain data.<br>Wrong is wrong for good.<br><b style="color:var(--ts-red)">Policy: hard block + dual sign-off</b></p></div>
    </div>
    <div class="ts-alert-box amber">
      <h3>Never let the agent self-escalate</h3>
      <p>An L1 task must not promote itself to L2. Authorization must be explicit, revocable, and time-boxed.</p>
    </div>
    <div class="ts-stripe-b"></div>
  ...
  <section class="slide">
    ...
    <div class="ts-kicker">Don't govern with docs · govern with code</div>
    <h2 class="ts-h2">Thirty lines of YAML,<br><span class="ts-highlight-red">red lines hard-blocked</span></h2>
    <pre class="ts-codebox"><span class="cm"># nevoflux-policy.yaml · compiled → SDK runtime guard</span>
<span class="kw">level_1_allow</span>:
  - tools: [<span class="st">write_draft</span>, <span class="st">render_canvas</span>, <span class="st">read_gbrain</span>]

<span class="kw">level_2_require_review</span>:
  - tools: [<span class="st">send_email_draft</span>, <span class="st">open_pr</span>, <span class="st">write_staging_gbrain</span>]
    reviewer: <span class="st">human</span>

<span class="kw">level_3_hard_block</span>:
  - tools: [<span class="st">send_real_email</span>, <span class="st">transfer_money</span>, <span class="st">delete_prod_gbrain</span>]
    unless: <span class="st">two_human_sign_off AND within_24h</span>

<span class="bad">forbidden_always</span>:
  - <span class="bad">"r&#109; &#45;rf /"</span>
  - <span class="bad">"dr&#111;p table gbrain"</span>
  ...
  <section class="slide">
    ...
    <h2 class="ts-h2">Our Q1 <span class="red">12 agent incidents</span></h2>
    <p class="ts-sub">Caught every one in staging. But each one could have hit production.</p>
    <svg viewBox="0 0 1040 360" style="width:100%;max-width:1040px;margin-top:18px" xmlns="http://www.w3.org/2000/svg">
      <g font-family="Inter,sans-serif" font-size="14" fill="#4a4955">
        ...
        <g transform="translate(720,60)">
          <rect x="0" y="0" width="16" height="16" fill="#e0314a"/><text x="24" y="13" font-weight="700">L3 irreversible (3)</text>
          <rect x="0" y="26" width="16" height="16" fill="#d97706"/><text x="24" y="39" font-weight="700">L2 needs review (4)</text>
          <rect x="0" y="52" width="16" height="16" fill="#067647"/><text x="24" y="65" font-weight="700">L1 recoverable (5)</text>
          <text x="0" y="100" font-size="13" fill="#8a8892">All stopped at runtime by nevoflux-policy,</text>
          <text x="0" y="118" font-size="13" fill="#8a8892">never reached prod. But 3 L3 cases were close.</text>
        </g>
      </g>
    </svg>
    <div class="ts-stripe-b"></div>
  ...
  <section class="slide">
    ...
    <h2 class="ts-h2">7 questions you <span class="red">must pass</span> before launch</h2>
    <div class="ts-checklist">
      <div class="ts-check ok"><div class="box">✓</div><div class="txt">Can it delete things? Is there a human review? Can it roll back within 60 seconds?</div></div>
      <div class="ts-check ok"><div class="box">✓</div><div class="txt">Can a prompt injection push it past its permissions? (ran the red-team prompt suite)</div></div>
      <div class="ts-check"><div class="box">!</div><div class="txt">Does it handle PII? Is there PII in the logs too?</div></div>
      <div class="ts-check ok"><div class="box">✓</div><div class="txt">When upstream/downstream fails, does it start mutating other GBrain resources?</div></div>
      <div class="ts-check"><div class="box">!</div><div class="txt">Will 100 concurrent agents deadlock the browser?</div></div>
      <div class="ts-check ok"><div class="box">✓</div><div class="txt">Can it stop <b>instantly</b> when wrong? (does the kill switch take effect within 2 seconds)</div></div>
      <div class="ts-check"><div class="box">!</div><div class="txt">Is someone on call during an incident? Does the runbook have an agent-specific section?</div></div>
    </div>
    <div class="ts-stripe-b"></div>
  ...
  <section class="slide">
    <div class="ts-stripe"></div>
    <div class="ts-chrome"><span class="ts-alert-tag green">start tonight</span><span class="ts-page">07 / 08</span></div>
    <h2 class="ts-h2">Do <span class="ts-highlight-red">three things</span> tonight</h2>
    <div class="ts-grid-3">
      <div class="ts-card"><div class="lbl">1 · tier it</div><h4>Tag your agent<br>L1/L2/L3</h4><p>List every tool, mark each with a tier. Anything untagged defaults to L3.</p></div>
      <div class="ts-card"><div class="lbl">2 · write policy</div><h4>policy.yaml<br>wired to runtime</h4><p>Don't trust "be careful" in the prompt — trust the hard block in the SDK execution layer.</p></div>
      <div class="ts-card"><div class="lbl">3 · kill switch</div><h4>A red button<br>that stops in 2 seconds</h4><p>The CTO and on-call must both know how to press it. Rehearse once.</p></div>
    </div>
    <div class="ts-alert-box green">
      <h3>Real safety is process, not prompt</h3>
      <p>Prompts get injected, process does not. — Put the guardrail in the layer that can't be talked out of it.</p>
    </div>
    <div class="ts-stripe-b"></div>
  ...
  <section class="slide">
    ...
    <div style="margin:auto 0">
      ...
      <h1 class="ts-h1" style="font-size:140px">thanks <span class="red">·</span> stay safe</h1>
      <p class="ts-sub" style="font-size:24px">policy.yaml template, red-team prompt list, incident postmortem template — grab them from the NevoFlux safety pack.</p>
    </div>
    <div class="ts-stripe-b"></div>
...
Fill these sections in order; each `<section class="slide">` is one 16:9 slide:
- Slide 1 (Cover): the alert tag, kicker, strike-through negation headline, sub line, and the headline alert box stating the scaled risk.
- Slide 2 (Section divider): chapter kicker + oversized headline + one-line framing for the first chapter.
- Slide 3 (Risk tiers): the three L1/L2/L3 cards (green / amber / red) plus an amber alert box warning against self-escalation.
- Slide 4 (Policy as code): the `ts-codebox` YAML block — keep the `cm` / `kw` / `st` / `bad` spans for syntax highlighting; `bad` marks forbidden commands.
- Slide 5 (Incident chart): the inline SVG stacked bar chart (L1 green bottom, L2 amber, L3 red top) plus a legend; edit the rect heights and the count labels.
- Slide 6 (Checklist): the red/green checklist — add `.ok` to a `.ts-check` for a passing green check, omit it for an open amber item.
- Slide 7 (CTA): three action cards + a green closing alert box.
- Slide 8 (Thanks): closing slide with oversized headline and a resource call-to-action.
The alert tag pill takes `amber` or `green` modifier classes to match each slide's tone; keep the `ts-page` counters and `ts-footer` labels in sync.
