---
slug: packs/design-pack/templates/deck-hermes-cyber
type: template
lang: en
category: slides
title: "NevoFlux Cyber Terminal Deck"
title_zh: "Cyber Terminal Deck"
description: "A dark CRT-terminal slide deck for reviewing CLI tools, agents, and dev workflows, with traces, diffs, and benchmarks."
tags: [cyber, terminal, review, cli, template]
sample_image: packs/design-pack/assets/templates/deck-hermes-cyber.svg
source: html-anything/deck-hermes-cyber
---

## Design guidance

A cyber terminal review deck for CLI tools, agents, and developer workflows — built to show real traces, diffs, and benchmarks rather than glossy marketing.

Layout and design details:
- Pure black background (`#0a0c10`) overlaid with a 56px cyber grid, plus a CRT vignette to darken the corners.
- A faux window chrome with red/yellow/green traffic-light dots, and a `$ prompt` style command-line title above each headline.
- Large mint-green headlines (`#7ed3a4`) in JetBrains Mono, with a soft neon glow text-shadow.
- Stroke-only (outline) bar charts and a blinking terminal cursor for a live-shell feel.
- Three tag tiers — amber, green, and red — to flag positives, neutrals, and warnings.
...
<title>NevoFlux Cyber Terminal</title>
...
<div class="deck">
  ...
  <section class="slide is-active">
    ...
    <div class="hc-chrome"><div class="dots"><span></span><span></span><span></span></div><div>~/nevoflux · zsh · 118x42 · 01:37:04</div></div>
    <div style="margin:auto 0">
      <p class="hc-prompt">whoami --nevoflux</p>
      <h1 class="hc-h1">NEVOFLUX<br>AGENT / v0.9.2<span class="hc-cursor"></span></h1>
      <p class="hc-lede">An AI browser with a GBrain knowledge base, Canvas apps, and a CLI agent that claims to run whole engineering tasks on its own.<br>Actually useful, or just more hype? I ran it for 72 hours straight to find out.</p>
      <div style="margin-top:26px">
        <span class="hc-tag">gbrain-core</span>
        <span class="hc-tag">mcp-native</span>
        <span class="hc-tag amber">72h-benchmark</span>
        <span class="hc-tag red">honest-review</span>
      </div>
    </div>
    <div class="hc-footer"><span>nevoflux-review · lewis · 2026</span><span>01 / 08</span></div>
  </section>
  ...
  <section class="slide">
    ...
    <div style="margin:auto 0">
      ...
      <p class="hc-lede">From <code style="color:var(--hc-amber)">brew install nevoflux</code> to the first GBrain query — 4 minutes 22 seconds total.</p>
    </div>
  ...
  <section class="slide">
    ...
    <h2 class="hc-h2">Out-of-the-box numbers</h2>
    <p class="hc-lede">cold start → first-successful-task, averaged over three runs</p>
    <div class="hc-grid-3">
      <div class="hc-card"><div class="lbl">install time</div><div class="val">42s</div><div class="desc">Single binary, no docker, no python env.</div></div>
      <div class="hc-card"><div class="lbl">first token</div><div class="val">1.8s</div><div class="desc">Wired to claude-opus-4-6, no warm-up.</div></div>
      <div class="hc-card"><div class="lbl">first canvas app</div><div class="val">4m22s</div><div class="desc">A simple GBrain-backed note viewer scaffolded end to end.</div></div>
    </div>
    <div class="hc-grid-2">
      <div class="hc-card"><div class="lbl">// verdict +</div><div class="val" style="color:var(--hc-green);font-size:18px">Cold start really is fast</div><div class="desc">Compared to the docker + pip flow of OpenClaw, it's faster by more than an order of magnitude.</div></div>
      <div class="hc-card"><div class="lbl">// verdict -</div><div class="val" style="color:var(--hc-red);font-size:18px">Pack/MCP setup isn't friendly</div><div class="desc">Env vars have to be hand-stuffed into ~/.nevoflux/env, and the docs barely mention it.</div></div>
    </div>
  ...
  <section class="slide">
    <div class="hc-grid"></div>
    <div class="hc-scanlines"></div>
    <div class="hc-chrome"><div class="dots"><span></span><span></span><span></span></div><div>trace · nevoflux run</div></div>
    <p class="hc-prompt">nevoflux run "build a canvas app from my gbrain notes"</p>
    <h3 class="hc-h3" style="margin-top:12px">↓ real trace (excerpt)</h3>
    <pre class="hc-codebox" style="margin-top:10px"><span class="cm"># nevoflux v0.9.2 · session 42a1</span>
[<span class="fn">plan</span>]    <span class="st">"query gbrain → pick design pack → scaffold canvas app"</span>
[<span class="fn">gbrain</span>]  search "release notes"      <span class="cm">// 214 hits</span>
[<span class="fn">read</span>]    packs/design-pack/seed.md   <span class="cm">// 88 lines</span>
[<span class="kw">think</span>]   <span class="st">"use feature-grid + landing-hero, bind to gbrain query"</span>
[<span class="fn">edit</span>]    apps/notes/canvas.tsx       <span class="hl">+43 -17</span>
[<span class="fn">edit</span>]    apps/notes/query.ts         <span class="hl">+12 -4</span>
[<span class="fn">test</span>]    pnpm vitest canvas          <span class="st">PASS 18/18</span>
[<span class="fn">commit</span>]  <span class="var">"feat(canvas): gbrain-backed notes app"</span>
[<span class="fn">push</span>]    origin feat/notes-canvas    <span class="st">ok</span>

<span class="cm"># total 3m 14s · 14k tokens · $0.21</span></pre>
  ...
  <section class="slide">
    <div class="hc-grid"></div>
    <div class="hc-scanlines"></div>
    <div class="hc-chrome"><div class="dots"><span></span><span></span><span></span></div><div>benchmark · nevoflux vs openclaw</div></div>
    <h2 class="hc-h2">72-hour comparison</h2>
    <p class="hc-lede">The same 48 GitHub issues, run once through each agent</p>
    <svg viewBox="0 0 1000 380" style="width:100%;max-width:1040px;margin-top:24px" xmlns="http://www.w3.org/2000/svg">
      <g font-family="JetBrains Mono, monospace" font-size="13" fill="#8a8892">
        ...
        <g transform="translate(820,50)">
          ...
          <text x="22" y="12" fill="#7ed3a4">nevoflux 0.9.2</text>
  ...
  <section class="slide">
    ...
    <p class="hc-lede" style="margin-top:14px">Worth installing, not yet worth depending on entirely.</p>
    <div class="hc-grid-2" style="margin-top:24px">
      <div class="hc-card"><div class="lbl">+ strong points</div><div class="desc">• GBrain-native core, fast cold start<br>• Highly readable traces<br>• Review-friendly diffs, decent commit messages</div></div>
      <div class="hc-card"><div class="lbl">- weak points</div><div class="desc">• Planner occasionally skips steps<br>• Canvas builds OOM on huge repos<br>• Pack/MCP config needs manual env</div></div>
    </div>
  ...
  <section class="slide">
    ...
    <h2 class="hc-h2">Want to run it yourself?</h2>
    <p class="hc-lede">Three commands, and in under 5 minutes you'll see it do its first thing.</p>
    <pre class="hc-codebox" style="margin-top:22px"><span class="cm"># 1. install</span>
<span class="kw">$</span> brew install nevoflux/tap/nevoflux

<span class="cm"># 2. auth (have your anthropic api key ready)</span>
<span class="kw">$</span> nevoflux auth login

<span class="cm"># 3. first task</span>
<span class="kw">$</span> cd ~/your-repo && nevoflux run <span class="st">"build a canvas app from my gbrain notes"</span></pre>
    <div style="margin-top:26px">
  ...
  <section class="slide">
    ...
    <div style="margin:auto 0">
      ...
      <p class="hc-lede">Full traces, the PR list for all 48 tasks, and the benchmark scripts all live at <span style="color:var(--hc-amber)">github.com/lewis/nevoflux-review</span></p>
    </div>
...
Eight 16:9 slides driving a CLI/agent/dev-tool review, in slide order:
1. Cover — `$ prompt` kicker, big mint headline with blinking cursor, lede, and a tag row (use the green/amber/red `hc-tag` tiers).
2. Section divider — oversized `// chapter` title and a one-line setup note.
3. Spec cards — a 3-up `hc-grid-3` of metric cards (label / value / desc), plus a 2-up `hc-grid-2` of `+`/`-` verdict cards.
4. Code/trace — a `$ prompt` command and an `hc-codebox` showing a real trace; use `cm`/`kw`/`st`/`fn`/`var`/`hl` spans for syntax color.
5. Chart — stroke-only SVG bar chart comparing your tool against a baseline; edit bar heights, labels, and the legend.
6. Stats/verdict — one `hc-big` score and two `hc-card` strong/weak lists.
7. CTA/install — an `hc-codebox` with install/auth/run commands and a closing tag row.
8. Thanks — `exit 0` prompt, big `// thanks` title, and a pointer to where the artifacts live.

Slots to fill per slide: the `hc-chrome` right-hand status text, the `hc-prompt` command, headings (`hc-h1`/`hc-h2`/`hc-h3`), `hc-lede` body copy, card label/value/desc, tags, and the `hc-footer` left label + page number. Keep everything self-contained — no external URLs beyond the placeholder text.
