---
slug: packs/design-pack/templates/mobile-app
type: template
lang: en
category: mobile
title: "NevoFlux Mobile App Screen"
title_zh: "iPhone App 单屏"
description: "Single-screen mobile app design rendered inside a pixel-accurate iPhone 15 Pro frame."
tags: [mobile, ios, app, phone, template]
sample_image: packs/design-pack/assets/templates/mobile-app.svg
source: html-anything/mobile-app
---
## Design guidance

A single-screen mobile app design placed inside a pixel-accurate iPhone 15 Pro frame. Intent: show one focused app screen as a believable phone screenshot.

Layout:
- Status bar (time + battery + signal).
- App header (title / avatar / search).
- Main content built around one clear archetype: feed, list, detail, or form.
- Bottom tab bar with 4-5 tabs.

Design details:
- Use a realistic dynamic island and keep safe-area padding at the top and bottom.
- The phone frame is a rounded black shell with a soft drop shadow; the screen inside has its own large corner radius and clips overflow.
- A warm, low-contrast neutral background with white surfaces and a single accent color used for the hero card, active states, and the selected tab.
- The hero card carries the primary action and the largest type on screen; supporting sections (stats grid, list rows) stay quiet and secondary.
...
<head>
  ...
  <title>NevoFlux — GBrain screen</title>
  <style>
    :root {
  ...
  <div class="frame" data-od-id="frame">
    <div class="screen">
      ...
      <div class="header" data-od-id="header">
        ...
        <h1>Two packs left to index.</h1>
      </div>
      <div class="timer-card" data-od-id="timer">
        <p class="label">Indexing GBrain</p>
      ...
      <div class="section" data-od-id="stats">
        ...
        <div class="stats">
          <div class="stat"><div class="v">3</div><div class="l">Canvas apps</div></div>
          <div class="stat"><div class="v">75</div><div class="l">Pages saved</div></div>
          <div class="stat"><div class="v">2</div><div class="l">Packs synced</div></div>
        </div>
      </div>
      <div class="tasks" data-od-id="tasks">
        <p class="section label" style="padding: 0;">Up next</p>
        <div class="task done"><div class="check"></div><div class="body"><div class="title">Index design-pack skills</div><div class="meta">25m · completed</div></div></div>
        <div class="task"><div class="check"></div><div class="body"><div class="title">Ship sync-engine Canvas app</div><div class="meta">2 sessions estimated</div></div></div>
        <div class="task"><div class="check"></div><div class="body"><div class="title">Wire agent SDK into GBrain</div><div class="meta">1 session</div></div></div>
      </div>
...
Fill these slots, keeping the layout fixed:
- Status bar: keep `9:41`, signal, and battery as the canonical iOS sample values.
- Header: `greeting` (date or context line) plus a short, punchy `h1` describing the current state.
- Hero card (`.timer-card`): the screen's primary archetype — swap the `label`, the large `countdown` value, the `progress` fill width, and the two action buttons (one neutral, one `.primary`).
- Stats grid (`.stats`): three small metrics, each a value (`.v`) and label (`.l`).
- List (`.tasks`): rows with a `.title` and `.meta`; add the `done` class to a row to show it checked and struck through.
- Tab bar (`.tabbar`): 4 tabs, exactly one carrying `active`; each is an emoji `icon` plus a one-word label.
