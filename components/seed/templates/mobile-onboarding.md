---
slug: packs/design-pack/templates/mobile-onboarding
type: template
lang: en
category: mobile
title: "NevoFlux Mobile Onboarding"
title_zh: "App 引导多屏"
description: "Three iPhone frames side by side showing a mobile onboarding flow: splash, value-prop, and sign-in."
tags: [onboarding, ios, signup, 引导, template]
sample_image: packs/design-pack/assets/templates/mobile-onboarding.svg
source: html-anything/mobile-onboarding
---
## Design guidance

Template: App onboarding, three screens. Intent: show three key mobile onboarding screens side by side so the whole first-run flow reads at a glance.

Layout:
- Phone 1 — Splash: logo / hero art plus a tagline.
- Phone 2 — Value-prop: an illustration, one supporting sentence, and dot indicators.
- Phone 3 — Sign-in: email / Apple / Google options plus a primary CTA.

Design details:
- Three identical phone frames in a CSS grid (`grid-template-columns: repeat(3, auto)`), each a rounded 44px device shell with an 8px dark bezel, a notch, and a status bar (9:41 · 5G).
- Warm canvas palette: cream stage background with soft radial gradients top and bottom, a single warm-orange accent reused for art, pager, and CTA glyphs.
- A serif display face for the stage title and screen headlines; a sans body for supporting copy and labels.
- Each phone body shares the same vertical rhythm: a top row (Skip / Help), a hero or heading block, body copy, a three-dot pager that advances per screen, and a CTA or sign-in stack pinned near the bottom.
- All artwork is inline SVG (gradient "moon", a card-with-check illustration); no external images.
- Responsive: below 1100px the three phones stack into a single column, each phone widening to 92vw (max 360px).
...
<title>NevoFlux · Mobile onboarding</title>
<style>
  :root {
    --canvas: #efece4;
    --ink: #232118;
...
<body>
<div class="stage">
  <h1>NevoFlux · onboarding flow</h1>
  <p class="lede">Three screens, one tone of voice. The first run that connects your browser, your GBrain knowledge base, and your Canvas apps.</p>
  ...
  <div class="phones">
    ...
    <div class="phone">
      ...
      <div class="phone-body">
        ...
        <div class="copy">
          <h2>A browser that thinks.</h2>
          <p>NevoFlux turns every page you read into searchable memory. No tab chaos, no lost links — just a workspace that remembers for you.</p>
        </div>
    ...
    <div class="phone">
      ...
      <div class="phone-body">
        ...
        <div class="copy">
          <h2>Your GBrain answers back.</h2>
          <p>Ask in plain words and the NevoFlux agent searches everything you've saved, then drafts the answer right inside a Canvas app.</p>
        </div>
    ...
    <div class="phone">
      ...
      <div class="phone-body">
        ...
        <div style="padding: 24px 0;">
          <h2 style="font-family: var(--display); font-size: 32px; margin: 0 0 8px; letter-spacing: -0.01em; line-height: 1.05; font-weight: 700;">Sync your GBrain.</h2>
          <p style="margin: 0; color: var(--muted); font-size: 15px;">Keep your knowledge base and packs across every device. We don't sell your data, ever.</p>
        </div>
        ...
        <div class="terms">By continuing, you agree to NevoFlux's <a>Terms</a> and <a>Privacy Policy</a>.</div>
...
- `stage` — the cream backdrop: `h1` flow title and a `lede` one-liner that frames the three screens.
- `phones` — a three-column grid; each `phone` is a self-contained device frame with `notch`, `statusbar`, and `phone-body`.
- Screen 1 (splash) — `phone-top` Skip link, a `hero` inline-SVG mark, a `copy` block (serif `h2` tagline + muted `p`), the `pager` (first dot active), and a dark `cta`.
- Screen 2 (value-prop) — same shape; swap the SVG illustration and the `copy` headline/sentence, advance the active `pager` dot to the middle.
- Screen 3 (sign-in) — a heading block, a `signin-options` stack (`opt apple` / `opt google` / `opt email`, each with a `glyph`), a spacer, the `terms` line, and the `pager` (last dot active).
- All artwork is inline SVG and the brand glyphs are CSS-only; no external URLs or images.
