---
slug: packs/design-pack/templates/deck-obsidian-claude-zh
type: template
lang: zh
category: slides
title: "GitHub Dark 紫渐变 Deck"
title_en: "NevoFlux Gradient Deck"
description: "GitHub 暗色底 + 紫蓝环境光 + 三色渐变标题 + GitHub 风代码块的开发者教程演示，已适配 NevoFlux 内容。"
tags: [github, dark, purple, mcp, agent, 模板]
...
类 GitHub Blog / Linear Changelog 风格的开发者教程 deck。

布局：
- GitHub 暗色底 (#0d1117) + 紫蓝 radial 环境光 + 60px 网格 mask（向边缘渐隐）。
- 居中布局，顶部带一枚紫色 pill tag。
- 三色渐变标题文字（#a855f7 → #60a5fa → #34d399）。
- GitHub 风代码 palette（绿/蓝/紫/橙 token）+ 紫色左边框高亮块。

设计细节：
- 卡片使用暗色面 (#161b22)，顶部带一条两端渐隐的紫色渐变发丝线。
- 步骤列表、双列与三列网格、超大渐变数字的统计计数、居中引言块，以及收尾的「Thanks.」页。
- 页码在右上角；全部自包含（无外部图片，只用渐变与内联 SVG）。
...
<title>NevoFlux × GBrain 渐变</title>
...
<div class="deck">
  ...
  <section class="slide is-active">
    ...
    <div class="oc-tag">● NEVOFLUX × GBRAIN · 第二大脑</div>
    <h1 class="oc-h1">把 NevoFlux 和 GBrain<br>拧成 <span class="oc-g">一条神经</span></h1>
    <p class="oc-sub">不是又一个 AI 笔记插件 —— 是让 NevoFlux 智能体真正理解你 GBrain 知识库的结构、链接、双向引用，<br>然后在你想写东西之前就把资料准备好。</p>
  ...
  <section class="slide">
    ...
    <h1 class="oc-h1" style="font-size:110px">Why <span class="oc-g">not</span> 云文档？</h1>
  ...
  <section class="slide">
    ...
    <h2 class="oc-h2">云文档 vs <span class="oc-g">GBrain</span> · 对 AI 友好度</h2>
    <div class="oc-grid-2">
      <div class="oc-card">
        <span class="oc-badge oc-bb">云文档</span>
        <h4 style="font-size:20px;margin-bottom:10px">数据库原生</h4>
        <p style="color:var(--oc-dim);font-size:14px;line-height:1.65">适合结构化任务、团队协作，但是——<br>• 智能体要走 API，拿不到实时全文<br>• 嵌套块结构复杂，token 成本高<br>• 本地化差，没法当长期记忆</p>
      </div>
      <div class="oc-card" style="border-color:rgba(168,85,247,.35);background:rgba(124,58,237,.05)">
        ...
        <p style="color:var(--oc-dim);font-size:14px;line-height:1.65">对 AI 天生友好 ——<br>• 所有东西就是文件，NevoFlux 智能体直接读<br>• 双链 = 天然 graph，抽实体几乎零成本<br>• 离线、可 git、可 diff、可回滚</p>
      </div>
    </div>
  ...
  <section class="slide">
    ...
    <div class="oc-steps">
      <div class="oc-step"><div class="oc-sn">1</div><div class="oc-sc"><h4>装 NevoFlux 浏览器 + 开 GBrain Local API</h4><p>内置开关，一个勾就开。它让外部进程能 read/write 你的 GBrain 知识库。</p></div></div>
      <div class="oc-step"><div class="oc-sn">2</div><div class="oc-sc"><h4>接 NevoFlux 智能体 + gbrain server</h4><p>一个配置文件就能接，token 填 GBrain 的 api key。</p></div></div>
      <div class="oc-step"><div class="oc-sn">3</div><div class="oc-sc"><h4>从 pack 装 5 个 design skills</h4><p>markdown / bases / canvas / cli / defuddle —— 让智能体知道怎么正确使用 GBrain。</p></div></div>
      <div class="oc-step"><div class="oc-sn">4</div><div class="oc-sc"><h4>让智能体自己整理一次</h4><p>「帮我把最近 10 篇笔记里的重复概念合并，生成一张新的 MOC」—— 90 秒出结果。</p></div></div>
    </div>
  </section>
    ...
    <pre class="oc-code"><span class="cm">// ~/.config/nevoflux/nevoflux.config.json</span>
{
  <span class="cc">"agents"</span>: {
    ...
    <p class="oc-sub" style="margin-top:18px">重启 NevoFlux 浏览器，输入 <b style="color:var(--oc-accent3)">/agents</b>，你会看到 gbrain 已连。</p>
  </section>
  ...
  <section class="slide">
    ...
    <h2 class="oc-h2">跑了 90 天，我的 <span class="oc-g">GBrain 数据</span></h2>
    <div class="oc-grid-3" style="margin-top:28px">
      <div class="oc-card" style="text-align:center"><div class="oc-big oc-g" style="font-size:80px">1,842</div><p style="color:var(--oc-dim);margin-top:8px;font-size:13px">知识库笔记数</p></div>
      <div class="oc-card" style="text-align:center"><div class="oc-big oc-g" style="font-size:80px">6.3k</div><p style="color:var(--oc-dim);margin-top:8px;font-size:13px">backlinks (由智能体自动补)</p></div>
    ...
    <div class="oc-hl" style="margin-top:26px">最大收益不是「AI 帮我写」，而是「AI 帮我把旧笔记重新连起来」—— 每周 30 分钟，GBrain 就会主动生长。</div>
  </section>
  ...
  <section class="slide">
    ...
    <div style="margin-top:36px">
      <span class="oc-pill">⬇ NevoFlux 浏览器</span>
      <span class="oc-pill">⬇ GBrain 知识库</span>
      <span class="oc-pill">⬇ NevoFlux 智能体 SDK</span>
  ...
  <section class="slide">
    ...
    <p class="oc-sub" style="margin-top:26px">配置模板、pack manifest、我的 GBrain 结构图都在 <b style="color:var(--oc-accent3)">NevoFlux design-pack</b> 里</p>
  </section>
...
八张自包含的 16:9 幻灯片，按以下插槽填写：
- 第 1 页（封面）：pill 标签、渐变主标题、副文、三枚特性 pill。
- 第 2 页（章节）：章节标签、超大标题（含一个渐变词）、副文。
- 第 3 页（对比）：两张卡片（徽章 + 标题 + 要点列表）+ 一个紫色高亮块作为关键洞察。
- 第 4 页（步骤）：编号步骤列表，每步带一个 `h4` 标题与一行说明。
- 第 5 页（代码）：GitHub 风代码块，使用 `cm`（注释）、`cc`（键）、`cs`（字符串）span；末尾一句操作提示。
- 第 6 页（统计）：三张统计卡，超大渐变数字，外加一句总结高亮。
- 第 7 页（引言/CTA）：引言块带署名 + 下载/CTA pill。
- 第 8 页（致谢）：收尾渐变词与一行资源指引。

保持所有 CSS 与类名不变，只替换可见文本。增删幻灯片时记得同步更新 `oc-snum` 页码。
