---
slug: packs/design-pack/templates/deck-dir-key-nav-zh
type: template
lang: zh
category: slides
title: "极简方向键 Keynote"
title_en: "Dir-Key Nav Minimal Deck"
description: "8 页单色背景的极简方向键 Keynote：160px 大标题 + 4px 短粗 accent 线 + Mono 箭头列表。"
tags: [minimal, kbd, monocolor, 模板]
...
「有话要说但没什么可看」的极简 keynote。整套 deck 全靠排版与颜色支撑，别无其他。

布局：
- 页数由内容决定（短内容 8 页起步，长内容应更多）。每页单色背景，从调色板里循环选取（靛 / 奶 / 绛 / 翠 / 灰 / 紫 / 白 / 炭），同色可复用。
- 160px display 大标题 + 4px 短粗 accent 线。
- 箭头 → 前缀的 Mono 列表。
- 左下角 ← → 键盘提示，右下角页码。
...
<div class="deck">
  ...
  <section class="slide t-indigo is-active">
    ...
    <div style="margin:auto 0">
      <div class="dk-eyebrow">NevoFlux 浏览器</div>
      <h1 class="dk-h0">标签页<br>装不下一个 <span class="dk-accent">大脑</span></h1>
      <span class="dk-line"></span>
      <p class="dk-lede">8 种背景、8 张幻灯，一个关于如何把浏览器变成「常驻知识库」的最短陈述。<strong>按 → 继续。</strong></p>
    </div>
  ...
  <section class="slide t-cream">
    ...
    <div style="margin:auto 0">
      ...
      <h1 class="dk-h0">问题 <span class="dk-accent">所在</span>.</h1>
      <span class="dk-line"></span>
      <p class="dk-lede">关掉标签页的那一刻，浏览器就把一切忘光。你读过的每个页面都没留下任何痕迹。</p>
    </div>
  ...
  <section class="slide t-crimson">
    ...
    <div style="margin:auto 0">
      ...
      <ul class="dk-list">
        <li>查了一下午资料，第二天每个链接都找不回来</li>
        <li>AI 助手在每一次对话里都从零开始</li>
        <li>笔记、标签页、书签彼此之间毫无关联</li>
        <li>上周学到的东西既搜不到，也再也想不起</li>
      </ul>
    </div>
  ...
  <section class="slide t-emerald">
    ...
    <div style="margin:auto 0">
      ...
      <h2 class="dk-h1">答案是一个<br><span class="dk-accent">GBrain</span>，<br>而不是更多标签页。</h2>
      <p class="dk-lede" style="margin-top:10px">NevoFlux 把每个页面、每段高亮、每次对话都<strong>写进本地知识库</strong>。<br>智能体在回答之前，先去读 GBrain。</p>
      <div class="dk-grid-2">
        <div class="dk-col"><h3>× 多开标签页</h3><p>堆一堆再也不会打开的标签，慢、重、最终丢失。</p></div>
        <div class="dk-col"><h3>✓ GBrain + 检索</h3><p>随手捕获，按需召回，结构化且永久可搜索。</p></div>
      </div>
    </div>
  ...
  retrieval: hybrid  # embedding + bm25</pre>
      <p class="dk-lede" style="margin-top:16px;font-size:20px">你现在拥有一个<strong>随浏览不断生长的知识库</strong>。每次对话它都会被读、被更新。</p>
    </div>
  ...
  <section class="slide t-violet">
    ...
    <div style="margin:auto 0">
      ...
      <p class="dk-lede" style="margin-top:14px;font-size:26px">的重复查找被消除。智能体从 GBrain 召回，平均每个任务节省 <strong>4.2 分钟</strong>。</p>
  ...
  <section class="slide t-white">
    ...
    <div style="margin:auto 0">
      ...
      <h2 class="dk-h1">构建<br>你的 <span class="dk-accent">GBrain</span>。</h2>
      <span class="dk-line"></span>
      <p class="dk-lede">不是再装一个插件。是决定：从今晚起，<strong>你访问的每个页面都汇入同一个知识库</strong>。</p>
      <pre class="dk-code" style="font-size:18px">$ nevoflux init ~/gbrain
$ nevoflux pack add design-pack
$ nevoflux canvas new "我的第一个应用"</pre>
    </div>
  ...
  <section class="slide t-charcoal">
    ...
    <div style="margin:auto 0">
      ...
      <span class="dk-line"></span>
      <p class="dk-lede">NevoFlux 的智能体 SDK 与完整的 pack 目录都随浏览器交付。欢迎按 ← 再看一遍。</p>
    </div>
...
每个 `<section class="slide t-...">` 是一页；`t-*` 类从调色板里选取该页背景。需要填写的插槽：
- `dk-snum`（右上角）：递增页码，如 `01 / 08`。
- `dk-eyebrow`：简短的大写标签（章节、栏目名或 kicker）。
- `dk-h0` / `dk-h1` / `dk-h2`：display 大标题；把关键词包进 `<span class="dk-accent">`。
- `dk-line`：4px accent 短线（无内容）。
- `dk-lede`：一两句正文，用 `<strong>` 强调。
- `dk-list`：箭头前缀的列表项（Mono 字体）。
- `dk-grid-2` / `dk-col`：两栏对比块。
- `dk-code`：小段 Mono 代码/配置片段。
- `dk-big` + `<svg>` 进度条：单个核心指标加进度条。
- `dk-keyhint`（左下角）与 `dk-page`（右下角）：键盘提示与页面标签。

增删 `<section>` 块即可改变页数，循环使用 `t-*` 调色板类。
