---
slug: packs/design-pack/templates/deck-blueprint-zh
type: template
lang: zh
category: slides
title: "蓝图架构 Deck"
title_en: "Knowledge Arch Blueprint"
description: "印刷友好的架构 / pipeline 讲解 deck：奶油纸底 + 蓝图网格 + 锈红强调 + 黑边硬卡片。"
tags: [blueprint, architecture, engineering, 模板]
...
一套认真、印刷友好的架构 / pipeline 讲解 deck。它刻意不用渐变和柔和阴影，让画面读起来像一张真正的工程蓝图。

布局：
- 奶油纸底色（`#F0EAE0`）+ 48px 蓝图网格 mask，用径向渐变遮罩让网格向边缘淡出。
- 横向的 Pipeline 步骤盒，其中一个「核心」盒抬高并填充锈红，标出焦点环节。
- 封面右上角钉一个锈红（`#B5392A`）insight callout。
- Playfair serif 大字号 + 内联 SVG 虚线反馈环示意图。

设计细节：
- 零渐变、零柔和阴影 —— 全部平涂填充、2px 黑色硬边、利落直角。
- 正文用两级墨色，锈红做强调，发丝线（`#cec8be`）用于分区标签与页脚。
- 代码盒与 manifest 用等宽字体（JetBrains Mono）；serif 仅用于超大主数字。
...
<div class="deck">
  ...
  <section class="slide is-active">
    ...
    <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:40px;margin-bottom:44px">
      <div>
        <div class="kb-kicker">NevoFlux Stack · 架构图 v2</div>
        <h1 class="kb-h1">GBrain <span class="rust">知识库</span> 的<br>工程化蓝图</h1>
        <p class="kb-sub">从「乱贴剪藏」到「可审计、可纠错、可复用」的第二大脑 —— 这是 NevoFlux 浏览器与 GBrain 知识库背后的一张系统图。</p>
      </div>
      <div class="kb-insight"><span class="kk">KEY INSIGHT</span>大多数知识库缺一块：<br>反馈闭环让错误能回流纠正。</div>
    </div>
    <div class="kb-section-label">Pipeline · End-to-end</div>
    <div class="kb-pipeline">
      <div class="kb-step"><div class="kb-step-num">STEP 01</div><div class="kb-step-title">采集</div><div class="kb-step-body">NevoFlux 浏览器剪藏、PDF、Podcast 转写、聊天记录</div></div>
      ...
      <div class="kb-step hero"><div class="kb-step-num">STEP 03 · CORE</div><div class="kb-step-title">入 GBrain</div><div class="kb-step-body">结构化成双链笔记，实体、关系、属性全在一张图里</div></div>
      <div class="kb-step"><div class="kb-step-num">STEP 04</div><div class="kb-step-title">使用</div><div class="kb-step-body">NevoFlux Agent 随时检索、回答、再写入</div></div>
    </div>
  ...
  <section class="slide">
    ...
    <h1 class="kb-h1" style="font-size:48px">静态 vault vs <span class="rust">GBrain v2</span></h1>
    <div class="kb-grid-2">
      <div class="kb-card">
        <div class="kb-kicker" style="color:#888">静态 vault</div>
        <h4>一次性写入</h4>
      ...
      <div class="kb-card" style="background:var(--kb-rust-soft);border-color:var(--kb-rust)">
        ...
        <h4>反馈闭环</h4>
        <p>NevoFlux Agent 使用 GBrain 时记录每次 miss / 幻觉 / 过期事实，自动回灌到源笔记，让 vault 会自我修正。</p>
      </div>
    </div>
    <div class="kb-legend">
  ...
  <section class="slide">
    ...
    <p class="kb-sub">一个 100 行的 NevoFlux Agent Skill，把「Agent 用 GBrain 顺不顺」回写成 vault 的一条条修订记录。</p>
...
<span class="kw">nightly</span>:
  ...
  <section class="slide">
    ...
    <h1 class="kb-h1" style="font-size:44px">GBrain v2 <span class="rust">跑了半年</span> 的数据</h1>
  ...
  <section class="slide">
    ...
    <h1 class="kb-h1" style="font-size:60px">开始你的 <span class="rust">GBrain v2</span></h1>
    <p class="kb-sub">不用重写所有笔记。先接一条回路，让 NevoFlux Agent 的每次使用都在「改好」你的 vault。</p>
    <div class="kb-pipeline">
      <div class="kb-step"><div class="kb-step-num">TONIGHT</div><div class="kb-step-title">装 Pack</div><div class="kb-step-body">pnpm i -g @nevoflux/gbrain-feedback</div></div>
      ...
      <div class="kb-step"><div class="kb-step-num">MONTH 1</div><div class="kb-step-title">开始信它</div><div class="kb-step-body">你的 GBrain 会变成活的 vault</div></div>
    </div>
  ...
  <section class="slide">
    ...
    <div style="margin:auto 0;text-align:center">
      ...
      <h1 class="kb-h1" style="font-size:140px;margin-top:24px">谢谢 <span class="rust">·</span> NevoFlux</h1>
      <p class="kb-sub" style="margin:0 auto;font-size:22px">图纸、Pack、笔记模板都在 <b>NevoFlux 设计包</b> 里</p>
    </div>
...
八页架构 deck，按下列插槽填写：
- 封面：`kb-kicker`（stack 标签）、`kb-h1`（标题，含一个 `.rust` span）、`kb-sub`（一句话前提），以及 `kb-insight` callout 放你最锋利的洞见。
- Pipeline：四个 `kb-step` 盒；用 `kb-step hero` 标出焦点环节。每个含 `kb-step-num`、`kb-step-title`、`kb-step-body`。
- 分区页：一个超大 `kb-h1` 加一句 `kb-sub`。
- 对比页：两个 `kb-card`（静态 vs 升级）加一组 `kb-legend` 说明节点与流向。
- 代码页：`kb-codebox`，用 `.cm` 注释、`.kw` 键、`.st` 字符串、`.hl` 高亮 token。
- 图示页：内联 SVG 反馈环 —— 把盒子和虚线箭头标签改成你自己的 pipeline。
- 数据页：一个 `kb-big-num` 加若干 `kb-card` 指标卡。
- CTA + Thanks：四步落地 pipeline 加一页收尾。

增删页时记得同步每个页脚的 `NN / 08` 计数。所有视觉都是 CSS 渐变或内联 SVG —— 没有任何外链图片。
