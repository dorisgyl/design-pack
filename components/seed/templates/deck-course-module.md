---
slug: packs/design-pack/templates/deck-course-module
type: template
lang: en
category: slides
title: "NevoFlux Course Module Deck"
title_zh: "课程 / 培训 Deck"
description: "Warm-paper teaching deck with a persistent learning-objectives sidebar and a built-in self-check MCQ slide."
tags: [course, workshop, training, 教学, template]
...
A teaching / workshop deck that keeps the learning objectives visible at all times. Use it for onboarding modules, internal training, and hands-on workshops.

Layout:
- Cover slide (module name + instructor).
- Learning-objectives list, pinned in the left sidebar and shown on every content slide.
- Body slides (concept + worked example).
- A multiple-choice self-check (MCQ) slide.
- Wrap-up with a preview of the next module.

Design details:
- Warm paper background with a Playfair Display serif for headings.
- Academic-but-friendly palette: teal accent, amber secondary, soft clay tertiary.
- The sidebar tracks progress with bullet states (done `●`, current `▸`, upcoming `○`).
...
<title>Module 04 · GBrain Retrieval · NevoFlux Academy</title>
...
<div class="deck">
  ...
  <section class="slide full" data-title="Cover">
    <p class="kicker">NEVOFLUX ACADEMY · MODULE 04</p>
    <h1 class="h1 mt-s">GBrain Retrieval: answers that <em>cite their source</em>.</h1>
    <p class="lede mt-l" style="max-width:62ch">In this module you'll learn how the NevoFlux browser turns your saved pages, notes, and Canvas apps into a GBrain knowledge base — and how the agent grounds every answer in what you actually captured.</p>
    <div class="row mt-l" style="gap:16px">
      <span class="pill-academic">~ 45 min read</span>
      <span class="pill-academic">prereq · GBrain basics, packs</span>
      <span class="pill-academic">tool · NevoFlux SDK</span>
    </div>
    <div class="deck-footer"><span>Maya Okonkwo · DevRel · 2026</span><span class="slide-number" data-current="1" data-total="7"></span></div>
  </section>
  ...
  <section class="slide" data-title="Objectives">
    <aside class="sidebar">
      ...
      <ul class="obj-list">
        <li class="current">Define GBrain retrieval</li>
        <li>Pick the right source scope</li>
        <li>Trace a grounded answer</li>
        <li>Tune chunking ↔ recall</li>
        <li>Know when retrieval helps</li>
      </ul>
    ...
    <div class="main">
      ...
      <div class="stack mt-l">
        <div class="concept-box"><h4>① Explain GBrain retrieval in one sentence.</h4><p class="dim">"The agent answers a question by fetching the most relevant chunks from your knowledge base before it writes anything."</p></div>
        <div class="concept-box"><h4>② Scope a query so it never invents sources.</h4><p class="dim">Every grounded answer points back to a captured page, note, or Canvas app.</p></div>
        <div class="concept-box"><h4>③ Trace a retrieval pipeline on paper.</h4><p class="dim">Given a question, sketch embed → search → rerank → answer.</p></div>
        <div class="concept-box"><h4>④ Convert a broad search into a scoped pack query.</h4><p class="dim">And explain when narrow beats broad.</p></div>
      </div>
    </div>
  </section>
  ...
  <section class="slide" data-title="Concept">
    <aside class="sidebar">
      ...
      <h5>Learning objectives</h5>
      <ul class="obj-list">
        <li class="done">Define GBrain retrieval</li>
        <li class="current">Pick the right source scope</li>
        <li>Trace a grounded answer</li>
        <li>Tune chunking ↔ recall</li>
        <li>Know when retrieval helps</li>
      </ul>
      <h5>Key terms</h5>
      <p class="dim" style="font-size:13px">chunk · embedding · source scope · rerank</p>
    </aside>
    <div class="main">
      <p class="kicker">CORE CONCEPT</p>
      <h2 class="h2 mt-s">Two parts, always.</h2>
      <p class="lede mt-m">A GBrain query has exactly two things inside it: a <b>source scope</b> (where to look) and a <b>retrieval step</b> (which chunks to pull before the agent answers).</p>
      <div class="callout">
        <b>Rule of thumb.</b> If you can't name the source scope out loud, don't run the query yet. Decide which pack or workspace it should read first.
      </div>
      <div class="grid g2 mt-l">
        <div class="concept-box"><h4>Source scope</h4><p class="dim">The slice of GBrain the query is allowed to read — one workspace, pack, or saved collection.</p><p class="pill-academic">e.g. <b>scope = "research-pack"</b></p></div>
        <div class="concept-box"><h4>Retrieval step</h4><p class="dim">Embed the question, find the nearest chunks, and hand only those to the agent.</p><p class="pill-academic">e.g. <b>top_k = 6</b></p></div>
      </div>
    </div>
  </section>
  ...
  <section class="slide" data-title="Example">
    <aside class="sidebar">
      ...
      <ul class="obj-list">
        <li class="done">Define GBrain retrieval</li>
        <li class="done">Pick the right source scope</li>
        <li class="current">Trace a grounded answer</li>
        <li>Tune chunking ↔ recall</li>
        <li>Know when retrieval helps</li>
      </ul>
      <h5>Try it yourself</h5>
      <p class="dim" style="font-size:13px">Open a Canvas app and run the snippet on the right. Then ask <code>ask("which pack covers billing?")</code>.</p>
    </aside>
    <div class="main">
      <p class="kicker">WORKED EXAMPLE</p>
      <h2 class="h2 mt-s">Grounded ask, 7 lines.</h2>
      <div class="code mt-m"><pre style="margin:0"><span class="cmt"># gbrain.ask() retrieves first, then the agent answers with citations</span>
...
<span class="kw">def</span> ask(q):
    ...
    <span class="kw">return</span> gbrain.answer(q, context=hits)   <span class="cmt"># grounded reply</span>

<span class="kw">print</span>(ask(<span class="str">"how do packs link skills?"</span>))   <span class="cmt"># → cited answer</span></pre></div>
      <div class="callout">
        <b>Trace the call.</b> question → embed(q) → search top 6 chunks in <code>design-pack</code> → rerank → answer(q, context) → reply with 6 source links = <b>grounded</b>.
      </div>
    </div>
  </section>
  ...
  <section class="slide" data-title="Exercise">
    <aside class="sidebar">
      ...
      <ul class="obj-list">
        <li class="done">Define GBrain retrieval</li>
        <li class="done">Pick the right source scope</li>
        <li class="done">Trace a grounded answer</li>
        <li class="current">Tune chunking ↔ recall</li>
        <li>Know when retrieval helps</li>
      </ul>
      <h5>Time</h5>
    ...
    <div class="main">
      ...
      <h2 class="h2 mt-s">Write <em>scoped_ask(q, pack)</em>.</h2>
      <p class="lede mt-m">Return a grounded answer that only reads one pack — no global GBrain search allowed.</p>
      <div class="exercise mt-l">
        ...
        <ol style="color:var(--text-2);line-height:1.8;margin:10px 0 0">
          <li>Set the source scope. What does <code>scope</code> equal for the "design-pack"?</li>
          <li>Retrieve the top 6 chunks with <code>gbrain.search</code>.</li>
          <li>Test it: a billing question should cite the billing pack, not the blog.</li>
          <li>Bonus: what happens if the pack is empty? Return a safe "no source found" reply.</li>
        </ol>
      </div>
      <p class="dim mt-m" style="font-size:14px">Stuck? Remember: a source scope is the smallest slice of GBrain that already contains the answer.</p>
    </div>
  </section>
  ...
  <section class="slide" data-title="Check">
    <aside class="sidebar">
      ...
      <ul class="obj-list">
        ...
        <li class="done">Pick the right source scope</li>
        <li class="done">Trace a grounded answer</li>
        <li class="done">Tune chunking ↔ recall</li>
        <li class="current">Know when retrieval helps</li>
      </ul>
      <h5>Self-assess</h5>
    ...
    <div class="main">
      ...
      <h2 class="h2 mt-s">Which query will hallucinate a source?</h2>
      <div class="mt-l">
        <div class="mcq"><div class="letter">A</div><div><b>ask(q, scope="design-pack", top_k=6)</b><p class="dim" style="font-size:13px;margin:4px 0 0">Scoped to one pack and retrieves before answering. Grounded.</p></div></div>
        <div class="mcq correct"><div class="letter">B</div><div><b>ask(q, context=None)</b><p class="dim" style="font-size:13px;margin:4px 0 0"><b style="color:var(--accent)">✓ Correct.</b> No retrieval, no context — the agent answers from memory and may invent sources.</p></div></div>
        <div class="mcq"><div class="letter">C</div><div><b>ask(q, scope="*", top_k=6)</b><p class="dim" style="font-size:13px;margin:4px 0 0">Broad, but still retrieves real chunks across GBrain. Grounded, just noisier.</p></div></div>
      </div>
    </div>
  </section>
  ...
  <section class="slide full" data-title="Summary">
    ...
    <div class="grid g2 mt-l">
      <div class="concept-box"><h4>✓ Define GBrain retrieval</h4><p class="dim">The agent fetches relevant chunks before it answers.</p></div>
      <div class="concept-box"><h4>✓ Scope a query safely</h4><p class="dim">Every grounded answer points back to a real source.</p></div>
      <div class="concept-box"><h4>✓ Trace a retrieval pipeline</h4><p class="dim">You can unwind embed → search → rerank → answer by hand.</p></div>
      <div class="concept-box"><h4>✓ Judge when to use it</h4><p class="dim">Knowledge questions → retrieval. Pure formatting → just the agent.</p></div>
    </div>
    <div class="callout mt-l">
      <b>Up next · Module 05.</b> Building a Canvas app on GBrain: we'll use everything you just learned — but inside a live NevoFlux app, not a snippet.
    </div>
  </section>
...
- Cover slide (`.slide.full`): set the module kicker, the serif `h1` headline, a one-paragraph lede, three `pill-academic` meta chips, and the footer (instructor + slide number).
- Sidebar (`.sidebar`, repeated on every content slide): edit `.brand`, the `Learning objectives` `.obj-list`, and the secondary block (key terms / progress / time). Toggle each `<li>` class between default (upcoming), `done`, and `current` to advance the tracker.
- Concept slides: use `.concept-box` cards, `.callout` for rules of thumb, and `.code` blocks with `.cmt` / `.kw` / `.str` spans for syntax color.
- Exercise slide: fill the `.exercise` block (auto-labeled "Exercise") with the task and an ordered checklist.
- Check slide: provide MCQ options as `.mcq` rows; mark the answer with `.mcq.correct`.
- Summary slide (`.slide.full`): four recap `.concept-box` cards plus a `.callout` previewing the next module.
- Keep `data-current` / `data-total` and the slide count in sync if you add or remove slides. The deck is fully self-contained — no external assets beyond the Google Fonts `@import`.
