---
slug: packs/design-pack/templates/deck-blueprint
type: template
lang: en
category: slides
title: "Knowledge Arch Blueprint"
title_zh: "蓝图架构 Deck"
description: "Print-friendly architecture and pipeline slide deck on cream paper with a blueprint grid, rust-red accents, and hard black-edged cards."
tags: [blueprint, architecture, engineering, template]
sample_image: packs/design-pack/assets/templates/deck-blueprint.svg
source: html-anything/deck-blueprint
---

## Design guidance

A serious, print-friendly deck for explaining an architecture or pipeline. It deliberately avoids gradients and soft shadows so it reads like a real engineering blueprint.

Layout:
- Cream paper background (`#F0EAE0`) overlaid with a 48px blueprint grid that is masked with a radial gradient so it fades toward the edges.
- A horizontal pipeline of step boxes, with one "core" box lifted up and filled rust-red to mark the focal stage.
- A rust-red (`#B5392A`) insight callout pinned to the top-right of the cover.
- Large Playfair serif display numerals plus an inline SVG dashed feedback-loop diagram.

Design details:
- Zero gradients, zero soft shadows — flat fills, hard 2px black borders, and crisp corners.
- Two ink tones for body text, a rust accent for emphasis, and a hairline rule (`#cec8be`) for section labels and the footer.
- Mono font (JetBrains Mono) for code boxes and manifests; serif only for the oversized hero number.
...
<div class="deck">
  ...
  <section class="slide is-active">
    ...
    <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:40px;margin-bottom:44px">
      <div>
        <div class="kb-kicker">NevoFlux Stack · Architecture v2</div>
        <h1 class="kb-h1">The <span class="rust">GBrain</span><br>Engineering Blueprint</h1>
        <p class="kb-sub">From scattered clippings to an auditable, self-correcting, reusable second brain — this is the system diagram behind the NevoFlux browser and its GBrain knowledge base.</p>
      </div>
      <div class="kb-insight"><span class="kk">KEY INSIGHT</span>Most knowledge bases miss one piece:<br>a feedback loop that lets errors flow back and get corrected.</div>
    </div>
    <div class="kb-section-label">Pipeline · End-to-end</div>
    <div class="kb-pipeline">
      <div class="kb-step"><div class="kb-step-num">STEP 01</div><div class="kb-step-title">Capture</div><div class="kb-step-body">Browser clips, PDFs, podcast transcripts, and chat history straight from the NevoFlux browser</div></div>
      <div class="kb-step"><div class="kb-step-num">STEP 02</div><div class="kb-step-title">Denoise</div><div class="kb-step-body">Strip nav bars, ads, duplicate blocks, and low-signal material</div></div>
      <div class="kb-step hero"><div class="kb-step-num">STEP 03 · CORE</div><div class="kb-step-title">GBrain-ify</div><div class="kb-step-body">Structure into linked notes — entities, relations, and properties all in one graph</div></div>
      <div class="kb-step"><div class="kb-step-num">STEP 04</div><div class="kb-step-title">Use</div><div class="kb-step-body">The NevoFlux agent retrieves, answers, and writes back at any time</div></div>
    </div>
  ...
  <section class="slide">
    ...
    <div style="margin:auto 0">
      ...
      <h1 class="kb-h1" style="font-size:120px">Why flat <span class="rust">notes</span><br>stopped scaling</h1>
      <p class="kb-sub" style="font-size:24px;margin-top:20px">When your knowledge outgrows your memory,<br>you don't need more files — you need a <b>navigable graph</b>.</p>
    </div>
  ...
  <section class="slide">
    ...
    <h1 class="kb-h1" style="font-size:48px">Static vault vs <span class="rust">GBrain v2</span></h1>
    <div class="kb-grid-2">
      <div class="kb-card">
        <div class="kb-kicker" style="color:#888">Static vault</div>
        <h4>Write-once</h4>
        <p>Capture → transcribe → archive, and a mistake stays a mistake. No loop, no correction — the more notes pile up, the messier it gets.</p>
      </div>
      <div class="kb-card" style="background:var(--kb-rust-soft);border-color:var(--kb-rust)">
        <div class="kb-kicker">GBrain v2</div>
        <h4>Feedback loop</h4>
        <p>As the NevoFlux agent uses GBrain it logs every miss, hallucination, and stale fact, then flows them back into the source notes so the vault corrects itself.</p>
      </div>
    </div>
    <div class="kb-legend">
      <div class="d"><span class="b"></span>Ordinary node</div>
      <div class="d"><span class="b rust"></span>Core node · feedback-loop entry</div>
      <div class="d">—— Data flow &nbsp;&nbsp; ┈┈ Feedback loop</div>
    </div>
  ...
  <section class="slide">
    ...
    <h1 class="kb-h1" style="font-size:48px">How the loop <span class="rust">is wired</span></h1>
    <p class="kb-sub">A ~100-line NevoFlux agent skill that turns "how well the agent used GBrain" into a stream of revision records on your vault.</p>
    <pre class="kb-codebox"><span class="cm"># packs/gbrain-feedback/SKILL.md</span>
<span class="kw">name</span>: gbrain-feedback
...
<span class="kw">on_miss</span>:    record(<span class="st">query, reason=</span><span class="hl">"not-in-gbrain"</span>)
...
<span class="kw">nightly</span>:
  ...
  <section class="slide">
    ...
    <h1 class="kb-h1" style="font-size:44px">The full feedback loop</h1>
    <svg viewBox="0 0 1200 520" style="width:100%;max-width:1200px;margin-top:20px" xmlns="http://www.w3.org/2000/svg">
      ...
      <g font-family="Inter, sans-serif" font-size="16" font-weight="700">
        ...
        <text x="670" y="210" text-anchor="middle" fill="#fff" font-size="20">GBrain</text>
  ...
  <section class="slide">
    ...
    <h1 class="kb-h1" style="font-size:44px">GBrain v2 <span class="rust">after six months</span></h1>
    <div style="display:grid;grid-template-columns:1.3fr 1fr 1fr;gap:24px;margin-top:28px;align-items:center">
      <div style="text-align:center"><div class="kb-big-num">13</div><p style="font-size:14px;color:#666;margin-top:6px;letter-spacing:.1em;text-transform:uppercase">Key optimizations · all shipped</p></div>
      <div class="kb-card"><h4 style="color:var(--kb-rust)">-62%</h4><p>Hallucination rate (vs. the no-feedback-loop build)</p></div>
      <div class="kb-card"><h4 style="color:var(--kb-rust)">+4.1×</h4><p>Single-query hit rate</p></div>
    </div>
    <div class="kb-grid-2" style="margin-top:18px">
      <div class="kb-card"><h4>227 auto-revisions</h4><p>189 were approved and merged by hand, 38 were rejected (all archived for audit).</p></div>
      <div class="kb-card"><h4>412 new notes</h4><p>Clustered from the miss log, each one traceable back to its source.</p></div>
    </div>
  ...
  <section class="slide">
    ...
    <h1 class="kb-h1" style="font-size:60px">Start your <span class="rust">GBrain v2</span></h1>
    <p class="kb-sub">No need to rewrite every note. Wire up one loop and let every agent retrieval quietly improve your vault.</p>
    <div class="kb-pipeline">
      <div class="kb-step"><div class="kb-step-num">TONIGHT</div><div class="kb-step-title">Install the pack</div><div class="kb-step-body">pnpm i -g @nevoflux/gbrain-feedback</div></div>
      <div class="kb-step"><div class="kb-step-num">DAY 2</div><div class="kb-step-title">Run it for 7 days</div><div class="kb-step-body">Watch the miss log build up on its own</div></div>
      <div class="kb-step hero"><div class="kb-step-num">DAY 8 · CORE</div><div class="kb-step-title">Review your first PR</div><div class="kb-step-body">Spend 15 minutes on the auto-generated revisions</div></div>
      <div class="kb-step"><div class="kb-step-num">MONTH 1</div><div class="kb-step-title">Start trusting it</div><div class="kb-step-body">Your GBrain becomes a living vault</div></div>
    </div>
  ...
  <section class="slide">
    ...
    <div style="margin:auto 0;text-align:center">
      ...
      <h1 class="kb-h1" style="font-size:140px;margin-top:24px">thanks <span class="rust">·</span> NevoFlux</h1>
      <p class="kb-sub" style="margin:0 auto;font-size:22px">Blueprints, packs, and note templates ship in the <b>NevoFlux design pack</b></p>
    </div>
...
Eight-slide architecture deck. Fill these slots:
- Cover: `kb-kicker` (stack label), `kb-h1` (title with one `.rust` span), `kb-sub` (one-line premise), and the `kb-insight` callout for your sharpest takeaway.
- Pipeline: four `kb-step` boxes; mark the focal stage with `kb-step hero`. Each has `kb-step-num`, `kb-step-title`, `kb-step-body`.
- Section divider: an oversized `kb-h1` plus a short `kb-sub`.
- Compare slide: two `kb-card` blocks (static vs. upgraded) and a `kb-legend` describing nodes and flows.
- Code slide: `kb-codebox` with `.cm` comments, `.kw` keys, `.st` strings, and `.hl` for the highlighted token.
- Diagram slide: the inline SVG feedback loop — rename the boxes and dashed-arrow labels to match your pipeline.
- Stats slide: one `kb-big-num` plus `kb-card` metric tiles.
- CTA + Thanks: a four-step rollout pipeline and a closing slide.

Keep every footer's `NN / 08` page counter in sync if you add or remove slides. All visuals are CSS gradients or inline SVG — no external image URLs.
