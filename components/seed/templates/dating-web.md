---
slug: packs/design-pack/templates/dating-web
type: template
lang: en
category: dashboard
title: "NevoFlux Signals Dashboard"
title_zh: "社区 / 配对数据墙"
description: "An editorial, consumer-feel metrics wall for a NevoFlux workspace: signal ticker, headline KPIs, 30-day activity bars, and a trend block."
tags: [dating, community, consumer, template]
...
Editorial "data wall" with a consumer-product feel: serif display typography paired with a monospace caption voice, on warm paper tones, and restrained, deliberate accent color.

Layout:
- Left rail navigation (brand, signed-in user, grouped link lists, live status).
- Ticker bar across the top for live signals.
- Headline KPIs in a 3-column stat grid.
- 30-day activity bar chart (inline SVG, last two bars highlighted in accent).
- Trend block with a short editorial lede.

Design details:
- Warm paper background (`--paper` / `--panel`), dark ink text, soft hairline rules (`--rule`).
- One accent (`--accent`, a burnt orange) plus semantic good/bad; never more than one loud color at a time.
- Big serif-display numbers with tabular figures (`font-feature-settings: 'tnum'`); monospace, uppercase, wide-tracked labels.
- Hairline borders separate every region instead of cards or shadows.
- Collapses from a two-column layout to a single column under 1100px; the KPI grid drops from three to two columns.
...
<head>
  ...
  <title>NevoFlux · your workspace, measured by what your agents ship</title>
  <style>
    :root {
      --paper: #f4ede0;
      --panel: #f9f3e7;
...
<body>
  <div class="ticker" data-od-id="ticker">
    <div class="left">
      <span>YOUR WORKSPACE, MEASURED BY WHAT YOUR AGENTS SHIP</span>
      <span style="opacity:0.6;">·</span>
      <span>SYNCED HOURLY</span>
    </div>
    <div>NEXT TIER AT <b>2,080 GBRAIN DOCS</b></div>
  </div>
  ...
  <div class="layout">
    <aside class="rail" data-od-id="rail">
      ...
      <div class="user">
        ...
        <div class="meta"><b>@samuel</b><span>RECALL 22.9 · TIER III</span></div>
      </div>
      ...
      <div>
        ...
        <ul>
          ...
          <li>review <span class="badge">3</span></li>
      ...
      <div>
        ...
        <ul>
          ...
          <li>GBrain &amp; packs</li>
          <li>archived <span style="font:11px/1 var(--mono);color:var(--muted);">14</span></li>
          <li>settings</li>
        </ul>
      </div>
      <div>
        ...
        <ul>
          <li>past releases</li>
          <li>stale canvases <span style="font:11px/1 var(--mono);color:var(--muted);">7</span></li>
        </ul>
      </div>

      <div class="status">
        <div class="live">online · last build 11m ago</div>
    ...
    <main data-od-id="main">
      <section class="grid" data-od-id="kpis">
        <div class="stat">
          <div class="label">GBrain docs indexed</div>
          <div class="value"><em>1,842</em></div>
          <p class="note"><span class="arrow">↑</span> 41 this wk · healthy growth.</p>
        </div>
        <div class="stat">
          <div class="label">Agent runs in 24h</div>
          <div class="value good">47</div>
          <p class="note">the tasks your agents shipped within a day.</p>
        </div>
        <div class="stat">
          <div class="label">Installed packs</div>
          <div class="value"><em>14</em></div>
          <p class="note">4 active · 7 dormant · 3 suggested.</p>
        </div>

        <div class="stat">
          <div class="label">Recall hit rate</div>
          <div class="value good">14%</div>
          <p class="note">above median for your workspace size.</p>
        </div>
        <div class="stat">
          <div class="label">Canvas apps shipped</div>
          <div class="value"><em>3</em></div>
          <p class="note">of 7 prototypes this year. you finish things.</p>
        </div>
        <div class="stat">
          <div class="label">Flaky skills in use</div>
          <div class="value bad">4</div>
          <p class="note">manageable. two are pinned for review.</p>
        </div>

        <div class="stat">
          <div class="label">Shared design tokens</div>
          <div class="value"><em>214</em></div>
          <p class="note">your single greatest consistency asset.</p>
        </div>
        <div class="stat">
          <div class="label">Avg build time</div>
          <div class="value"><em>2.1<span style="font-size:32px;">s</span></em></div>
          <p class="note">snappy. the browser keeps the SDK warm.</p>
        </div>
        <div class="stat">
          <div class="label">Offline hrs</div>
          <div class="value bad">4</div>
          <p class="note">/ 168 this wk. sync stayed green.</p>
        </div>
      </section>

      <section class="panel" data-od-id="bars">
        <div class="panel-head">
          <h3>agent runs — <em>last 30 days</em></h3>
          <div class="meta">↑ TRENDING UP · +3 NEW CANVAS APPS THIS MONTH · TWO FROM THE SAME PACK</div>
        </div>
      ...
      <section class="panel lower-panel" data-od-id="trend">
        <div class="panel-head">
          <h3>recall hit rate — <em>last 12 weeks</em></h3>
          <div class="meta">STEADY CLIMB FROM 8% → 14%. ATTRIBUTABLE TO ONE PACK INSTALL (DESIGN-PACK, WK 4).</div>
        </div>
        <p class="lede">A real climb, not a vibe. <b>One pack install</b> moved your recall hit rate more than four months of prompt tweaks — keep feeding GBrain from that pack, index more, guess less.</p>
      </section>
    </main>
  </div>
</body>
...
- `ticker` — the strip of live signals; left side is the tagline, right side is the next-tier target.
- `rail` — left navigation: `brand`, signed-in `user` (handle, recall ratio, tier), grouped link lists (Today / You / Archive) with count badges, and a live `status` line at the bottom.
- `kpis` — a 3x3 grid of headline stats. Each `stat` has a monospace `label`, a serif-display `value` (add `good` / `bad` for color, wrap a number in `em` for italic), and an italic `note`.
- `bars` — the 30-day activity chart. Set bar height via SVG `rect` `y`/`height`; highlight the most recent bars by giving them `fill="#c14a2b"`. Update the `axis` labels to match the range.
- `trend` — a closing block: a `panel-head` with an uppercase `meta` caption plus a single editorial `lede` paragraph (bold the key driver).
