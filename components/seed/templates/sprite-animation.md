---
slug: packs/design-pack/templates/sprite-animation
type: template
lang: en
category: poster
title: "NevoFlux Sprite Animation"
title_zh: "像素动画解说"
description: "A single explainer frame in pixel / 8-bit style with kinetic display type, looping in pure CSS so it can be screen-recorded - here telling a NevoFlux origin beat."
tags: [pixel, 8-bit, retro, explainer, template]
...
Template: pixel / 8-bit animated explainer.

Intent
- A single-frame poster for an educational, animated explainer. The whole thing loops with pure CSS `@keyframes` - no JavaScript - so it records cleanly to video.

Layout
- Full-bleed cream stage.
- A bold display number / large figure anchoring one corner.
- A single pixel-art mascot at the focal point (inline SVG or pure-CSS drawing).
- Kinetic display lettering (large CJK or other display glyphs that animate in and out).
- A timeline ribbon scrolling continuously along the bottom.

Design details
- Drive every motion with `@keyframes`; do not depend on JavaScript.
- Retro palette: red / cream / ink-green.
...
<head>
  ...
  <title>NevoFlux, 2021 — sprite animation</title>
...
<body>
  <div class="stage" data-od-id="stage">
    ...
    <div class="topbar" data-od-id="topbar">
      <div class="slug">
        <span class="jp">起源の一帧</span>
        <span class="en">EP. 01 · NEVOFLUX ORIGINS</span>
      </div>
      <div class="progress">
    ...
    <div class="year-label" data-od-id="year-label">CHAPTER 01 · A BROWSER WITH A BRAIN</div>
    ...
    <div class="sprite-stack" data-od-id="sprite">
      <div class="sprite-card">
        <!-- GBrain knowledge node sprite — pixel-art neuron card -->
        <svg width="120" height="160" viewBox="0 0 24 32" xmlns="http://www.w3.org/2000/svg" aria-label="GBrain knowledge node">
          <!-- background -->
        ...
        <div class="sprite-tag">知<small>GBRAIN · KNOWLEDGE</small></div>
      </div>
    </div>

    <div class="caption" data-od-id="caption">
      <strong>NevoFlux started as a browser that remembers.</strong>
      Built so every page you open feeds GBrain, the local knowledge base. Canvas
      apps and the agent SDK wouldn&rsquo;t ship for another <em>two</em> years.
    </div>

    <div class="ribbon" data-od-id="ribbon">
      <div class="ribbon-track">
        <div class="tick"><span class="dot"></span><span class="label">2021</span><span class="note">GBRAIN · BROWSER CORE</span></div>
        <div class="tick"><span class="dot"></span><span class="label">2022</span><span class="note">CANVAS APPS</span></div>
        <div class="tick"><span class="dot"></span><span class="label">2023</span><span class="note">AGENT SDK</span></div>
        <div class="tick"><span class="dot"></span><span class="label">2024</span><span class="note">PACK REGISTRY</span></div>
        <div class="tick"><span class="dot"></span><span class="label">2025</span><span class="note">DESIGN SKILLS</span></div>
        <div class="tick"><span class="dot"></span><span class="label">2026</span><span class="note">DESIGN PACK 1.0</span></div>
        <div class="tick"><span class="dot"></span><span class="label">NOW</span><span class="note">BUILD ANYTHING</span></div>
...
- `topbar` slug / progress: the episode label, step dots, `NN / NN` counter, and the blinking REC badge. Set the on-air step with the `.on` class on one dot.
- `kana`: two large display glyphs that animate in and out - swap for your focal word (here GBrain "knowledge core").
- `year-label` + `year`: the chapter caption and the big display figure (a milestone year). The red `.glitch` layer mirrors the same number for the flicker effect, so keep both copies identical.
- `sprite-stack` / `sprite-card`: the bobbing pixel mascot. Replace the inline SVG with your own pixel art and update the `sprite-tag` glyph plus its caption line.
- `caption`: the explainer paragraph - an italic `strong` lead-in plus one supporting sentence.
- `ribbon`: a scrolling timeline. Each `.tick` has a label and a note; duplicate the full set once (the track is doubled so the loop is seamless).
