---
slug: packs/design-pack/templates/deck-tech-sharing
type: template
lang: en
category: slides
title: "NevoFlux Tech Sharing Deck"
title_zh: "技术分享 Deck"
description: "GitHub-dark engineering talk deck with JetBrains Mono terminal code blocks, agenda, and Q&A for internal NevoFlux tech sharing."
tags: [tech talk, conference, engineering, template]
...
A deck for an internal engineering tech-sharing session or a conference talk. The tone is code-forward and dark.

### Layout
- Cover - the topic + speaker + handle.
- Agenda page.
- Several body pages - terminal code blocks plus the key takeaway for each.
- Demo page - a terminal screenshot.
- Q&A page.

### Design details
- GitHub-dark palette with JetBrains Mono: background `#0d1117`, surfaces `#161b22` / `#1c2230`, text `#e6edf3` / `#8b949e`, accents `#7ee787` (green), `#79c0ff` (blue), `#ff7b72` (red), `#d2a8ff` (purple).
- The kicker uses JetBrains Mono and is prefixed with a `>` shell prompt; section labels read like file names (`agenda.toml`, `mini-runtime.ts`).
- Terminal cards have a window bar with three traffic-light dots and a file name; code is syntax-highlighted with `.kw` / `.fn` / `.str` / `.cmt` / `.num` spans.
- Agenda rows are dashed-underlined mono rows: a green number, the topic, and a time estimate.
- 16:9 aspect. Self-contained single file; no external URLs - all imagery is CSS gradients or inline markup.
...
<title>How NevoFlux Schedules Agent Work · Tech Sharing</title>
...
<div class="deck">
  ...
  <section class="slide" data-title="Cover">
    ...
    <h1 class="h1 anim-fade-up" data-anim="fade-up">How NevoFlux<br>decides what the <span style="background:var(--grad);-webkit-background-clip:text;background-clip:text;color:transparent">agent does next</span>?</h1>
    <p class="lede mt-m">From a <span class="mono">GBrain</span> query to the Canvas app it builds — the agent loop, end to end.</p>
  ...
  <section class="slide" data-title="Agenda">
    ...
    <div class="stack mt-l">
      <div class="agenda-row"><span class="num">01</span><span class="t">Context: why the browser needs an agent</span><span class="d">~5min</span></div>
      <div class="agenda-row"><span class="num">02</span><span class="t">Deep dive 1: GBrain &amp; retrieval</span><span class="d">~12min</span></div>
      <div class="agenda-row"><span class="num">03</span><span class="t">Deep dive 2: the agent scheduler</span><span class="d">~15min</span></div>
      <div class="agenda-row"><span class="num">04</span><span class="t">Code: a Canvas app in ~40 lines</span><span class="d">~8min</span></div>
  ...
  <section class="slide" data-title="Context">
    ...
    <h2 class="h2">Problem: a chat box alone<br>can't drive a whole browser.</h2>
    <div class="grid g3 mt-l">
      <div class="card card-accent"><h4>One-shot prompt</h4><p class="dim">Ask once, get text back. No memory of your tabs, no way to act on the page.</p><span class="tag mt-s">❌ too shallow</span></div>
      <div class="card card-accent"><h4>Hard-coded macros</h4><p class="dim">Scripted clicks break the moment a site changes. Brittle and painful to maintain.</p><span class="tag mt-s">😩 brittle</span></div>
      <div class="card card-accent"><h4>Agent + GBrain</h4><p class="dim">Reads your local knowledge base, plans steps, builds Canvas apps on the fly.</p><span class="tag mt-s">✅ NevoFlux picks this</span></div>
    </div>
  </section>
  ...
  <section class="slide" data-title="Deep Dive 1">
    ...
    <h2 class="h2">GBrain is just one method, really.</h2>
    <div class="grid g2 mt-l" style="align-items:start">
      <div>
        <p class="lede">Everything you save — pages, notes, packs — becomes embeddings in your local <span class="mono">GBrain</span>. The agent does one thing: <span class="mono">recall</span> the right chunks, then ground its answer in them.</p>
        <div class="mt-l">
...
<span class="cmt">// recall() → top-k local chunks, no cloud</span>
<span class="cmt">// ground() → answer cites those chunks</span></pre>
      </div>
    </div>
  </section>
  ...
  <section class="slide" data-title="Deep Dive 2">
    ...
    <h2 class="h2">The agent is a small task scheduler.</h2>
    <div class="grid g2 mt-l" style="align-items:start">
      <div>
        <p class="lede">The agent loop = a queue of steps. Each tick it picks the next step, runs a tool, and feeds the result back into the plan until the goal is done.</p>
        <div class="stack mt-m">
          <div class="tag">✦ step queue · 256 slots</div>
          <div class="tag">✦ tool registry (read · click · build)</div>
          <div class="tag">✦ GBrain recall before every plan</div>
          <div class="tag">✦ Canvas slot for the live app</div>
        </div>
      </div>
      <div class="card" style="padding:32px">
        ...
        <div class="stack mt-m" style="font-family:'JetBrains Mono',monospace;font-size:14px;line-height:1.9;color:var(--text-2)">
          <div><span style="color:var(--accent)">1.</span> pop the next planned step</div>
          <div><span style="color:var(--accent)">2.</span> else recall context from GBrain</div>
          <div><span style="color:var(--accent)">3.</span> else call a tool (read / click / build)</div>
          <div><span style="color:var(--accent)">4.</span> else render into the Canvas app</div>
          <div><span style="color:var(--accent)">5.</span> else stop and ask the user</div>
        </div>
      </div>
    </div>
  </section>
  ...
  <section class="slide" data-title="Code">
    ...
    <h2 class="h2">A Canvas app the agent can build.</h2>
...
<span class="kw">export default</span> <span class="fn">defineCanvas</span>({
    title: <span class="str">"Reading list"</span>,
    <span class="kw">async</span> <span class="fn">setup</span>(ctx) {
        <span class="kw">const</span> brain = <span class="fn">useGBrain</span>(ctx);
        <span class="kw">const</span> items = <span class="kw">await</span> brain.<span class="fn">recall</span>(<span class="str">"saved this week"</span>, <span class="num">12</span>);
        <span class="kw">return</span> { items };
    },
    <span class="fn">render</span>({ items }) {
        <span class="kw">return</span> items.<span class="fn">map</span>((it) =&gt; <span class="fn">Card</span>(it.title)); <span class="cmt">// that's the whole app</span>
    },
});</pre>
    </div>
  </section>
  ...
  <section class="slide" data-title="Takeaways">
    ...
    <div class="grid g3 mt-l">
      <div class="card card-accent"><h4>1 · GBrain stays local</h4><p class="dim">Embeddings and recall run in your browser. No cloud keys, your data never leaves.</p></div>
      <div class="card card-accent"><h4>2 · The plan is the pulse</h4><p class="dim">The agent doesn't act blindly; each tick it re-plans from fresh GBrain context.</p></div>
      <div class="card card-accent"><h4>3 · Canvas is the output</h4><p class="dim">Don't dump raw text. Let the agent <span class="mono">defineCanvas</span> a real little app instead.</p></div>
    </div>
    <p class="lede mt-l">Further reading: <span class="mono">docs.nevoflux.app/agent-loop</span> · <span class="mono">docs.nevoflux.app/gbrain</span></p>
  </section>
...
Fill these sections and slots; keep all CSS, class names, sizing, and structure unchanged - only swap the visible text.

- Cover (slide 1): `.kicker` is the `tech-sharing / date` stamp; the `.h1` is the talk title (wrap a key phrase in the gradient `span` for emphasis); `.lede` is the one-line subtitle; the `.speaker` block holds the handle, team, and runtime; the `.deck-footer` carries the hashtags and `data-current` / `data-total`.
- Agenda (slide 2): set the `agenda.toml` kicker, the page title, and one `.agenda-row` per topic (number / topic / time estimate).
- Context (slide 3): a headline framing the problem plus three `.card.card-accent` options, each ending with a `.tag` verdict.
- Deep dives (slides 4-5): left column is the prose explanation with inline `.tag` chips; right column is a `.terminal` code block or a `.card` step list. Recolor code with `.kw` / `.fn` / `.str` / `.cmt` / `.num` spans.
- Code (slide 6): one full-width `.terminal` with a file name in the window bar.
- Takeaways (slide 7): three `.card.card-accent` cards plus a `.lede` line of further reading.
- Q&A (slide 8): centered `?` glyph, a heading, your contact line, and `.tag` chips for the slides / code links.
