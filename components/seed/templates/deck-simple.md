---
slug: packs/design-pack/templates/deck-simple
type: template
lang: en
category: slides
title: "Simple Deck"
title_zh: "通用 Simple Deck"
description: "A clean, general-purpose horizontal-swipe HTML deck for NevoFlux pitches, overviews, and study walkthroughs."
tags: [deck, simple, swipe, template]
...
A clean, general-purpose horizontal-swipe deck for pitches, overviews, or study walkthroughs. Avoid a magazine tone - keep it plain and functional.

### Layout
- A cover slide, then N content slides, then a closing slide. Let N follow the length of the content and cover every key point: short content starts at 6-10 slides, longer content should use more.
- One core idea per slide, plus a single image or chart.
- A progress bar / counter pinned at the edge.

### Design details
- Arrow keys (left / right) switch slides, with hash sync so the URL tracks the current slide.
...
<head>
  ...
  <title>NevoFlux · Investor deck — Q2 2026</title>
  <style>
    :root {
...
<body>
  <section class="slide title" data-od-id="slide-1">
    <div class="eyebrow" style="color:#c96442;">NevoFlux · Series B · Q2 2026</div>
    <h1>The browser is the workspace.</h1>
    <p class="body">An agent-native browser with a GBrain knowledge base built in. Trusted by 3,184 paying teams.</p>
  </section>
  <section class="slide" data-od-id="slide-2">
    <div class="eyebrow">Problem</div>
    <h2>Your knowledge is scattered across forty tabs.</h2>
    <p class="body">Research lives in the browser, but every other tool treats it as throwaway. Teams re-find the same pages, re-read the same docs, and lose the context an agent needs to actually help. The browser remembers nothing.</p>
  </section>
  <section class="slide big-stat" data-od-id="slide-3">
    <div class="number">38×</div>
    <div class="caption">faster to recall a saved source from GBrain vs. searching tabs by hand, on real customer workloads.</div>
  </section>
  <section class="slide" data-od-id="slide-4">
    <div class="eyebrow">Why now</div>
    <h2>Three shifts make this market real.</h2>
    <div class="grid-3">
      <div class="pt"><h3 class="h">Agents in the loop</h3><p class="p">Work now flows through agents that need durable context. A browser that forgets is a non-starter.</p></div>
      <div class="pt"><h3 class="h">Canvas apps</h3><p class="p">Teams build internal Canvas apps in hours. They need a knowledge base and an SDK underneath, not another silo.</p></div>
      <div class="pt"><h3 class="h">Packs and skills</h3><p class="p">Design packs and reusable skills turn one-off prompts into shipped product. Distribution finally has a home.</p></div>
    </div>
  </section>
  <section class="slide" data-od-id="slide-5">
    <div class="quote-mark">"</div>
    <p class="quote-text">NevoFlux pays for itself in the first month. We were going to build an internal knowledge tool from scratch — instead our team just lives in the browser now.</p>
  ...
  <section class="slide title" data-od-id="slide-6">
    ...
    <h1>$22M to ship the agent-native browser.</h1>
    <p class="body">18-month runway, hire 14, expand the GBrain SDK and Canvas app marketplace.</p>
  </section>
...
- `.slide.title` — dark cover and closing slides. Set the `.eyebrow` kicker, an `h1` headline, and a one-line `.body` lead.
- `.slide` (default) — a content slide: `.eyebrow` label, an `h2` heading carrying the single core idea, and a `.body` paragraph.
- `.slide.big-stat` — one hero metric: `.number` for the figure, `.caption` for the explanation.
- `.grid-3` — three equal-weight points, each a `.pt` block with an `.h` title and a `.p` description.
- Quote slide — `.quote-mark`, `.quote-text` for the pull quote, `.quote-author` for attribution.
- Duplicate slides to fit the content; the `#counter` and arrow-key navigation pick up the new count automatically. Keep one idea (and at most one image or chart) per slide.
