---
slug: packs/design-pack/templates/live-dashboard
type: template
lang: en
category: dashboard
title: "NevoFlux Live Team Dashboard"
title_zh: "Notion 风团队仪表板"
description: "Notion-style team overview: KPI cards, a 7-day sparkline, a real-time activity feed, and a linked task database — all backed by seed data so it works offline."
tags: [notion, team, live, dashboard, template]
sample_image: packs/design-pack/assets/templates/live-dashboard.svg
source: html-anything/live-dashboard
---
## Design guidance

A Notion-style live overview of a team's activity. Intent: a real-time status board that still renders convincingly even with no data source bound — seed data is the fallback. Best viewed as a long desktop page.

Layout:
- Header with the workspace/team name and a time window (breadcrumbs plus a status pill).
- A grid of KPI cards.
- A 7-day sparkline trend.
- A real-time activity feed (avatar + action + relative time).
- A linked database task table (zebra rows + status pills).

Design details:
- Mimic the Notion look: callout blocks, toggle-style rows, and database-table coloring.
- Warm off-white surfaces, low-contrast neutral ink, and a single blue accent used for the sparkline, links, and the primary button.
- KPI cards carry an uppercase label, a large tabular-numerics value, and a small green/red delta line.
- Status pills use soft tinted backgrounds (done / in progress / blocked / to do / in review).
- When no connector is bound the live pill stays a static grey "Sample data" badge so the artifact never falsely claims a real data source; refreshes only tween the seeded fixtures.
- Respect `prefers-reduced-motion`: animations, value tweens, and row-flash highlights all short-circuit.
...
<div class="app">
  ...
  <aside class="side">
    <div class="ws" role="button" tabindex="0">
      ...
      <div class="ws-name">NevoFlux Studio</div>
    ...
    <div class="nav">
      ...
      <div class="nav-item" role="button" tabindex="0">
        <span class="nav-icon">✅</span><span>Agent runs</span>
        <span class="nav-caret">›</span>
      </div>
      <div class="nav-item" role="button" tabindex="0">
        <span class="nav-icon">🧠</span><span>GBrain</span>
        <span class="nav-caret">›</span>
      </div>
      <div class="nav-item" role="button" tabindex="0">
        <span class="nav-icon">🎨</span><span>Canvas apps</span>
      </div>
      <div class="nav-item" role="button" tabindex="0">
        <span class="nav-icon">📦</span><span>Packs</span>
      </div>
    </div>

    <div class="side-section">Private</div>
    <div class="nav">
      <div class="nav-item" role="button" tabindex="0"><span class="nav-icon">🗒️</span><span>Build journal</span></div>
    ...
    <div class="page">
      ...
      <div class="page-meta">
        <div class="updated">
          ...
          <span>Last edited by <strong>Sora Chen</strong></span>
        </div>
        <span>·</span>
        ...
        <button class="btn primary" id="refreshBtn" title="Pull latest from GBrain (sample mode — refreshes seeded data)">
          <span class="ico" id="refreshIco">↻</span>
          <span>Refresh from GBrain</span>
        </button>
      </div>
      ...
      <div class="callout">
        ...
        <div>
          ...
          The shape mirrors what the <strong>NevoFlux Studio</strong> workspace
          would return through the GBrain connector — but the numbers, names,
      ...
      <div class="kpis">
        ...
        <div class="kpi">
          <div class="label">Canvas apps awaiting review</div>
      ...
      <div class="two-col">
        ...
        <div class="card">
          <div class="card-title">
            ...
            <span>From <span style="color:var(--accent-ink)">GBrain</span></span>
          </div>
      ...
      <div class="footer">
        <span>Source: GBrain API · workspace <strong>nevoflux.app</strong></span>
        <span>·</span>
...
<div class="toast" id="toast">Synced — 3 changes from GBrain</div>

<script>
(function(){
  // ---------- Fake data state ----------
  const PEOPLE = [
    { id: "SC", name: "Sora Chen",    color: "#f1c40f" },
    { id: "MP", name: "Milo Park",    color: "#3498db" },
    { id: "LZ", name: "Lin Zhao",     color: "#e67e22" },
    { id: "DT", name: "Dana Torres",  color: "#9b59b6" },
    { id: "AN", name: "Aki Nakamura", color: "#1abc9c" },
    { id: "RP", name: "Ravi Patel",   color: "#e74c3c" }
  ];

  const ROW_POOL = [
    { icon: "🚀", title: "Q3 roadmap for the NevoFlux browser",  status: "progress", who: "SC", due: "May 12", prio: "High" },
    { icon: "🎨", title: "Canvas apps gallery v2 spec",          status: "review",   who: "LZ", due: "May 14", prio: "Med"  },
    { icon: "🐛", title: "Fix GBrain indexer timeout on large PDFs", status: "blocked",  who: "MP", due: "May 09", prio: "High" },
    { icon: "📊", title: "Agent SDK pricing experiment readout", status: "todo",     who: "DT", due: "May 18", prio: "Low"  },
    { icon: "✍️", title: "Launch blog draft · Live Artifacts",   status: "progress", who: "AN", due: "May 11", prio: "High" },
    { icon: "🔐", title: "Rotate GBrain connector token",        status: "todo",     who: "RP", due: "May 20", prio: "Med"  },
    { icon: "🧪", title: "QA the design-pack onboarding flow",   status: "review",   who: "MP", due: "May 16", prio: "Med"  },
    { icon: "📦", title: "Ship the analytics design skill pack", status: "progress", who: "LZ", due: "May 13", prio: "Low"  }
  ];
  ...
  const ACTIVITY_TEMPLATES = [
    ...
    (p, t) => ({ who: p, action: "shared",   target: t, suffix: "with the GBrain channel", icon: "🔗" }),
    (p, t) => ({ who: p, action: "added",    target: t, suffix: "to sprint",  icon: "📌" })
  ];

  const SEED_FEED = [
    { who: "Sora Chen",    action: "moved",      target: "Q3 roadmap for the NevoFlux browser", suffix: "to In progress", icon: "🚀", mins: 3 },
    { who: "Milo Park",    action: "commented on", target: "Fix GBrain indexer timeout on large PDFs", suffix: "",        icon: "💬", mins: 11 },
    { who: "Lin Zhao",     action: "created",    target: "Canvas apps gallery v2 spec",        suffix: "in Docs",        icon: "🎨", mins: 26 },
    { who: "Aki Nakamura", action: "shared",     target: "Launch blog draft · Live Artifacts", suffix: "with the GBrain channel", icon: "🔗", mins: 41 },
    { who: "Dana Torres",  action: "updated",    target: "Agent SDK pricing experiment readout", suffix: "properties",   icon: "✏️", mins: 58 }
  ];
  ...
  async function refresh({silent = false} = {}) {
    ...
    // Simulate latency to a GBrain API call.
...
- `side` — sidebar with the workspace switcher, search, and nav. The `Workspace` group lists Team Dashboard, Agent runs, GBrain, Canvas apps, and Packs; a `Connections` row shows the bound GBrain connector.
- `topbar` — breadcrumbs plus the live status pill. The pill is sticky `sample` (grey, static) until a real connector binds; only then does it escalate to `Live · synced` or `Stale · …`.
- `page-meta` — "last edited by" line, an `Auto · on/off` toggle, and the primary `Refresh from GBrain` button.
- `callout` — disclosure that the page runs on seeded fixtures, not a real workspace.
- `kpis` — four KPI cards (Total tasks, Done this week, Active members, Canvas apps awaiting review); each has an uppercase `label`, a big `value`, and an up/down `delta`.
- `two-col` — left card holds the 7-day sparkline (edit `series` and `dayLabels` to reshape it); right card holds the activity feed (`SEED_FEED`).
- `db` — linked task table fed by `ROW_POOL`; status pills are `done` / `progress` / `blocked` / `todo` / `review`.
- `PEOPLE` / `ROW_POOL` / `SEED_FEED` / `kpi` — seed-data arrays at the top of the script; rewrite these to change all names, tasks, and metrics without touching markup.
