---
slug: packs/design-pack/templates/web-proto-soft
type: template
lang: en
category: prototype
title: "Apple-tier Soft Prototype"
title_zh: "Apple Soft 原型"
description: "Apple-flavored soft landing prototype: silver/cream canvas, double-bezel cards, button-in-button CTAs, and spring motion."
tags: [apple, soft, squircle, spring, template]
sample_image: packs/design-pack/assets/templates/web-proto-soft.svg
source: html-anything/web-proto-soft
---
## Design guidance

An Apple-tier soft-material prototype page. Intent: a calm, premium product landing that leans on squircle geometry, spring-eased motion, and an ambient mesh background.

Layout:
- Silver / cream canvas with a slow-drifting ambient mesh background (two blurred radial blobs).
- Double-bezel cards: an outer shell with a soft highlight wraps an inner core panel, giving a layered, beveled feel.
- Button-in-button CTAs: a pill button that nests a circular icon chip inside it.
- Spring motion: hover states use a slightly over-shooting spring easing for a gentle bounce.

Design details:
- Neutral, low-contrast silver/cream surfaces with a single deep-sage accent reused across the brand mark, dots, rings, and code highlights.
- Floating pill navigation that sticks to the top, with a translucent blurred (frosted) backdrop.
- The hero pairs an oversized headline (with a muted "soft" continuation span) and a double-bezel preview card showing a small live status bento.
- A bento grid of double-bezel cards mixes light and one dark card; cards lift on hover with deeper shadows.
- Inline SVG arrow glyphs and CSS-only rings/conic-gradients keep the page fully self-contained (no external image URLs).
- A logo marquee scrolls horizontally and pauses on hover; a dark inverted closing band wraps the final CTA.
- Respects `prefers-reduced-motion` by disabling drift, reveal, and pulse animations.
...
<head>
  ...
  <title>NevoFlux — a calmer browser for AI-native work</title>
...
<body>
  ...
  <div class="nav-shell" data-od-id="topnav">
    <header class="nav">
      <span class="brand"><span class="brand-mark"></span>NevoFlux</span>
      <ul>
        <li><a href="#bento">Platform</a></li>
        <li><a href="#metrics">GBrain</a></li>
  ...
  <main>
    <section class="wrap hero reveal" data-od-id="hero">
      <div class="hero-copy">
        <span class="eyebrow"><span class="dot"></span>Public Beta · 2026 · Pack 12</span>
        <h1>A calmer browser for the work your agents <span class="soft">are already doing.</span></h1>
        <p class="lede">NevoFlux is an AI-native browser where your GBrain knowledge base, Canvas apps, and the built-in agent live in one place. Stop wiring tabs, prompts, and scratch files together by hand — NevoFlux keeps context, tools, and output in sync.</p>
        <div class="actions">
          <button class="pill">
            <span>Open NevoFlux</span>
            <span class="icon-wrap"><svg viewBox="0 0 12 12" stroke-width="1.6"><path d="M2.5 6h7M6 2.5L9.5 6 6 9.5"/></svg></span>
          </button>
          <button class="ghost">
            Read the SDK guide
        ...
        <div class="core">
          ...
          <div class="preview-grid">
            <div class="pv-cell span2">
              <div class="meta">agent · live</div>
              <div class="row" style="justify-content: space-between;">
                <div>
                  <h4 style="font-size: 14px;">canvas-agent / research</h4>
                  <div class="row" style="margin-top: 4px;"><span class="pulse"></span><p style="margin:0; font-size: 11px;">healthy · 7d 14h session</p></div>
                </div>
                <div class="ring" aria-hidden="true"></div>
              </div>
            </div>
            <div class="pv-cell">
              <div class="meta">gbrain / docs</div>
              <div class="num">2,418<small>notes</small></div>
            </div>
            <div class="pv-cell">
              <div class="meta">canvas runs / 24h</div>
              <div class="num">1,284<small>ok</small></div>
            </div>
            <div class="pv-cell span2">
              <div class="meta">activity · last entry</div>
              <p style="font-size: 11.5px; line-height: 1.6;">
                <code style="font-family: var(--mono); color: var(--accent);">04:12:09</code> · canvas <code style="font-family: var(--mono);">DESIGN-PACK</code> rendered by <b>canvas-agent/research</b> from <b>gbrain://product/specs</b>. Built in 41.7s.
              </p>
            </div>
          </div>
        </div>
    ...
    <section class="block reveal" id="bento">
      <div class="wrap">
        <div class="section-head">
          ...
          <p>Five primitives. No framework lock-in. Bring your own model, your own data, your own packs — keep the browser, the knowledge base, and the agent in one window.</p>
        </div>
        <div class="section-head" style="margin-bottom: 56px;">
          <h2>The browser is the boring part. We're <em style="font-weight:500; color: var(--muted); font-style: normal;">obsessed</em> with the boring part.</h2>
          <span></span>
        </div>
        ...
        <div class="bento">
          <div class="b-card span-3 row-2">
            <div class="core">
              <div class="meta">01 · gbrain</div>
              <h3>A knowledge base, not a pile of tabs.</h3>
              <p>GBrain gives every workspace a long-lived, searchable memory of your docs, links, and notes — portable across Canvas apps and agent sessions. The agent reads from it instead of re-scraping the web each run.</p>
              <div class="codeblock">
                <span class="c">// shared across canvas + agent</span><br>
                <span class="k">gbrain</span>.query = {<br>
                &nbsp;&nbsp;space: <span class="s">"product/specs"</span>,<br>
                &nbsp;&nbsp;scope: <span class="s">"pack:design-pack"</span>,<br>
                &nbsp;&nbsp;recall: nevoflux.memory(<span class="s">"q7"</span>),<br>
                };
              </div>
            </div>
          </div>
          <div class="b-card span-3">
            <div class="core">
              <div class="meta">02 · canvas</div>
              <h3>One canvas. One context. One render.</h3>
              <p>Canvas apps build live pages and tools from your GBrain context. NevoFlux re-renders before a stale draft becomes a copy-paste mess.</p>
              <div class="stat"><span class="big">0.41s</span><span class="unit">avg / render</span></div>
            </div>
          </div>
          <div class="b-card span-3 dark">
            <div class="core">
              <div class="meta">03 · agent</div>
              <h3>An agent that lives in the page.</h3>
              <p>Long-running. Tool-aware. Reads your GBrain, drives your Canvas apps, and leaves a trail. Every step, every prompt, every tool call, every edit — in one timeline you can actually scroll back through.</p>
            </div>
          </div>
          <div class="b-card span-2">
            <div class="core">
              ...
              <p>Isolated micro-runtime per Canvas app. Boots in 110ms.</p>
            </div>
          </div>
          <div class="b-card span-2">
            <div class="core">
              <div class="meta">05 · packs</div>
              <h3>Composable packs.</h3>
              <p>Drop in a pack of design skills and templates. Updated nightly.</p>
            </div>
          </div>
          <div class="b-card span-2">
            <div class="core">
              ...
              <p>Python, TypeScript, Go. <code style="font-family: var(--mono); color: var(--accent);">nevoflux</code> & <code style="font-family: var(--mono); color: var(--accent);">nf</code>.</p>
            </div>
          </div>
        </div>
      </div>
    ...
    <section class="marquee reveal">
      <div class="marquee-track">
        <span class="marquee-item">GBrain<em>·</em>Canvas<em>·</em>Agent SDK<em>·</em>Design Pack<em>·</em>Skills<em>·</em>Packs<em>·</em>Templates<em>·</em>Seeds<em>·</em>Runtime<em>·</em>CLI<em>·</em></span>
        <span class="marquee-item" aria-hidden="true">GBrain<em>·</em>Canvas<em>·</em>Agent SDK<em>·</em>Design Pack<em>·</em>Skills<em>·</em>Packs<em>·</em>Templates<em>·</em>Seeds<em>·</em>Runtime<em>·</em>CLI<em>·</em></span>
      </div>
    </section>

    <section class="closing reveal" data-od-id="closing">
      <div class="closing-inner">
        <h2>Less duct tape between your context and your output.</h2>
  ...
  <footer>
    <div class="row">
      <span>NevoFlux Browser · SOC 2 · ISO 27001 · v2026.05</span>
...
- `topnav` — pill nav: brand name, 3-4 section links, and a primary CTA pill.
- `hero` — eyebrow (stage/version), oversized headline with a muted `.soft` continuation span, a lede paragraph, a primary pill CTA, and a ghost secondary button.
- `hero-preview` — double-bezel preview card holding a small live bento: a status cell (agent name + pulse + ring), two metric cells (`num` + `small` unit), and an activity log line.
- `bento` — six double-bezel cards on a 6-column grid; reuse `.span-*` / `.row-2` for sizing and add `.dark` to one card for contrast. Each card has a `meta` kicker, an `h3`, body copy, and an optional `stat`, `codeblock`, or inline `code` accent.
- `marquee` — duplicate the same item list twice (second copy `aria-hidden`) so the loop is seamless.
- `closing` — inverted dark band with a headline, one supporting line, and a single CTA pill.
- `footer` — left: product/compliance/version string; right: a row of inline links.
- Keep all metrics, names, and labels coherent but illustrative; no external image/font assets are required for the visuals.
