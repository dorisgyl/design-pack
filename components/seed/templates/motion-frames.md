---
slug: packs/design-pack/templates/motion-frames
type: template
lang: en
category: video
title: "NevoFlux Motion Frames"
title_zh: "动效英雄帧"
description: "A single loopable hero frame built from pure-CSS motion: a rotating type ring, an abstract globe, a ticking timer, and parallax labels - a NevoFlux video intro or landing splash."
tags: [motion, title card, loop, video poster, template]
...
Intent
- A single hero frame carrying looping motion, usable as a video intro card or a landing-page splash.

Layout
- Rotating type ring built from concentric SVG / `transform`-driven circles, with editorial labels co-rotating around the outer ring.
- An animated globe (or abstract geometry) spinning slowly at the center.
- A ticking timer / signal line in a monospace font (JS optional - CSS alone can carry it).
- Parallax labels floating in the corners (issue number, meta block, top/bottom chrome rails).

Design details
- All motion is pure CSS, smooth and seamlessly loopable - no media files, no external assets.
- Cinematic, editorial color grading on a warm paper base, with exactly one neon-ish accent (here a burnt-orange `#c0563b`) used sparingly for the focal dot and one accent word.
- Keep the negative space generous; the rings and globe do the talking. The headline types in once via a `letter-spacing` ease-in.
...
<head>
  ...
  <title>Browse every source — NevoFlux</title>
  <style>
    :root {
      --paper: #f3eee5;
      --ink: #1a1816;
...
<body>
  <main data-od-id="stage">
    <div class="chrome top">
      <div>NEVOFLUX LABS — APR 17 · 2026</div>
      <div class="right"><div class="rule"></div><div>HOW GBRAIN WORKS</div></div>
    </div>

    <div class="meta-tl">
      <b>27 · BROWSER × GBRAIN</b>
      BUILD · 2026
    </div>
    <div class="issue">
    ...
    <div class="headline" data-od-id="headline">
      <span class="em">Browse</span> every <span class="accent">source.</span>
    </div>
    <div class="baseline"></div>

    <div class="chrome bot">
      <div>GBRAIN · LIVE</div>
      <div>INDEXING / 0001</div>
    </div>
  </main>
</body>
</html>
...
- `chrome.top`: left rail is a dateline / lab tag (`NEVOFLUX LABS — date`), right rail is a thin rule plus a short section label (e.g. `HOW GBRAIN WORKS`). Swap to your own product line.
- `meta-tl` (top-left): a bolded title line plus a sub-line - here a NevoFlux module reference (`BROWSER × GBRAIN` / `BUILD · 2026`).
- `issue` (top-right): a big italic figure (`.num`) plus an uppercase caption - drop in a fake metric (`9.4K` sources indexed, Canvas apps shipped, pack count, etc.).
- `composition`: the four `.ring` layers, the SVG `.globe`, and the `.focal-dot` are decorative - leave the geometry untouched; they carry the loop.
- `ring-labels`: optional `<span class="l1">…</span>` editorial words can be added inside, co-rotating with the outer ring.
- `headline` (bottom): the payoff line. Keep the `.em` / plain / `.accent` three-part structure - here `Browse / every / source.` with the accent word in burnt orange. Replace with your own short hero line.
- `chrome.bot`: a status pair (`GBRAIN · LIVE` / `INDEXING / 0001`). Everything is pure CSS/SVG - no external URLs, seamlessly loopable.
