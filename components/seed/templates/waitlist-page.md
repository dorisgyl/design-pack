---
slug: packs/design-pack/templates/waitlist-page
type: template
lang: en
category: prototype
title: "NevoFlux Waitlist Page"
title_zh: "等候名单页"
description: "Minimal pre-launch landing page with email capture, logo, and decorative layers for an early-access waitlist."
tags: [waitlist, launch, 预发布, template]
sample_image: packs/design-pack/assets/templates/waitlist-page.svg
source: html-anything/waitlist-page
---
## Design guidance

A minimal waitlist landing page for a new product or early-access beta. Intent: a single, focused page whose only job is to capture an email and build anticipation.

Layout:
- Centered composition: brand logo (top-left) plus a one-line tagline and a large hero headline that says clearly what the product does.
- Email-capture form: a name input, an email input, and a submit button merged into a single pill-style row.
- Below the form, the page leans on the decorative band rather than a feature grid; keep three optional micro selling-points (icon + one line) if you need to add proof.
- Bottom: a founders' note and social links, here expressed as a scrolling marquee ticker.

Design details:
- Decoration: an SVG gradient mesh / noise texture, a perspective grid floor, a coil row, an accent stripe, and a single star motif.
- On successful submit, show a micro-interaction (the form is replaced by a check / confirmation message).
- Warm peach background with a single dark ink color for text and a coral accent; display type is set in a bold geometric sans.
- Respects `prefers-reduced-motion` (the ticker stops) and collapses the form to a stacked single column under 600px.
...
<head>
  ...
  <title>NevoFlux — Coming Soon</title>
...
<body>
  ...
  <section class="upper">
    <h1 class="headline">NevoFlux is<br>almost here</h1>
    <p class="subtext">The browser where your knowledge does the work — GBrain remembers, Canvas apps build themselves, and the agent runs the rest.</p>
    ...
    <p class="success-msg" id="success-msg" role="status">You're on the GBrain early-access list! We'll be in touch.</p>
  </section>
...
Slots to fill:
- Logo — swap the `logo-svg` monogram and `logo-name` for the launching product or sub-brand (a pack, a Canvas app, a design skill).
- Headline — keep it to two short lines that name the product and signal "almost here".
- Subtext — one sentence on what it does (GBrain knowledge base, Canvas apps, the agent/SDK).
- Form — adjust input fields and the button label (e.g. "Request Early Access", "Join the Beta"); the success message is shown after a valid submit.
- Ticker — replace the repeated "COMING SOON" units with your own launch phrase; keep enough copies to fill the 50% scroll loop.
- Decoration (coil row, stripe, grid floor) is purely visual — recolor via the `--deco*` CSS variables, no content needed.
