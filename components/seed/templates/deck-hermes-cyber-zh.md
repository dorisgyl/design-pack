---
slug: packs/design-pack/templates/deck-hermes-cyber-zh
type: template
lang: zh
category: slides
title: "Cyber Terminal Deck"
title_en: "NevoFlux Cyber Terminal Deck"
description: "黑底 CRT 终端风格的测评幻灯片，适合 CLI 工具 / agent / 开发流程评测，含 trace、diff 与 benchmark。"
tags: [cyber, terminal, review, cli, 模板]
...
一套面向 CLI 工具、agent 与开发流程的赛博终端测评 deck，目标是展示真实的 trace、diff 与 benchmark，而不是华丽的营销页。

布局与设计细节:
- 纯黑底色（`#0a0c10`）叠加 56px 赛博网格，再加 CRT 暗角让四角变暗。
- 仿窗口 chrome 红绿灯三色圆点，每个标题上方用 `$ prompt` 命令行样式作为引子。
- 薄荷绿大字标题（`#7ed3a4`），JetBrains Mono 字体，带柔和霓虹辉光的文字阴影。
- 仅描边（stroke-only）的柱状图，配合闪烁的终端光标，营造实时 shell 的质感。
- 琥珀 / 绿 / 红三档 tag，用来标注正向、中性与警示信息。
...
<div class="deck">
  ...
  <section class="slide is-active">
    ...
    <div style="margin:auto 0">
      ...
      <p class="hc-lede">一个内置 GBrain 知识库、Canvas 应用和命令行 agent 的 AI 浏览器，号称能「自主跑完整软件工程任务」。<br>真的好用？还是又一轮营销？—— 我连续跑了 72 小时，告诉你答案。</p>
  ...
  <section class="slide">
    ...
    <div style="margin:auto 0">
      ...
      <p class="hc-lede">从 <code style="color:var(--hc-amber)">brew install nevoflux</code> 到第一次 GBrain 查询 —— 一共 4 分 22 秒。</p>
    </div>
  ...
  <section class="slide">
    ...
    <div class="hc-grid-3">
      ...
      <div class="hc-card"><div class="lbl">first canvas app</div><div class="val">4m22s</div><div class="desc">跑的是一个基于 GBrain 的简单笔记 Canvas 应用。</div></div>
    </div>
    <div class="hc-grid-2">
      ...
      <div class="hc-card"><div class="lbl">// verdict -</div><div class="val" style="color:var(--hc-red);font-size:18px">Pack/MCP 配置不够友好</div><div class="desc">env 变量需要手动塞进 ~/.nevoflux/env，文档几乎没写。</div></div>
    </div>
  ...
  <section class="slide">
    ...
    <p class="hc-prompt">nevoflux run "用我的 gbrain 笔记搭一个 canvas app"</p>
...
[<span class="fn">plan</span>]    <span class="st">"查 gbrain → 选 design pack → 生成 canvas app"</span>
...
[<span class="kw">think</span>]   <span class="st">"用 feature-grid + landing-hero，绑定到 gbrain 查询"</span>
...
<span class="cm"># 总耗时 3m 14s · 14k tokens · $0.21</span></pre>
  ...
  <section class="slide">
    ...
    <div class="hc-grid-2" style="margin-top:24px">
      <div class="hc-card"><div class="lbl">+ strong points</div><div class="desc">• GBrain 原生内核，冷启快<br>• trace 可读性极强<br>• diff 审核友好，commit message 也写得合格</div></div>
      <div class="hc-card"><div class="lbl">- weak points</div><div class="desc">• plan 阶段偶尔跳步<br>• 超大仓库下 canvas 构建会 OOM<br>• Pack/MCP 配置需要手动塞 env</div></div>
    </div>
...
<span class="kw">$</span> cd ~/your-repo && nevoflux run <span class="st">"用我的 gbrain 笔记搭一个 canvas app"</span></pre>
    <div style="margin-top:26px">
  ...
  <section class="slide">
    ...
    <div style="margin:auto 0">
      ...
      <p class="hc-lede">完整 trace、48 个任务的 PR 列表、benchmark 脚本都在 <span style="color:var(--hc-amber)">github.com/lewis/nevoflux-review</span></p>
    </div>
...
八张 16:9 幻灯片，用于 CLI / agent / 开发工具测评，按页序:
1. 封面 —— `$ prompt` 引子、带闪烁光标的薄荷绿大标题、lede 简介，以及一排 tag（用绿 / 琥珀 / 红三档 `hc-tag`）。
2. 章节分隔 —— 超大的 `// chapter` 标题加一行 setup 说明。
3. 规格卡 —— 一组 3 列 `hc-grid-3` 指标卡（label / value / desc），再加一组 2 列 `hc-grid-2` 的 `+`/`-` 结论卡。
4. 代码 / trace —— 一条 `$ prompt` 命令加一个 `hc-codebox` 展示真实 trace；用 `cm`/`kw`/`st`/`fn`/`var`/`hl` 这些 span 上语法色。
5. 图表 —— 仅描边的 SVG 柱状图，将你的工具与基线对比；改柱高、标签和图例即可。
6. 数据 / 结论 —— 一个 `hc-big` 评分加两张 `hc-card` 列出优点 / 缺点。
7. CTA / 安装 —— 一个 `hc-codebox` 列出 install / auth / run 命令，再配一排收尾 tag。
8. 致谢 —— `exit 0` 引子、超大 `// thanks` 标题，以及产物所在位置的指引。

每页需要填的槽位: `hc-chrome` 右侧状态文字、`hc-prompt` 命令、标题（`hc-h1`/`hc-h2`/`hc-h3`）、`hc-lede` 正文、卡片的 label/value/desc、tag，以及 `hc-footer` 左侧标签 + 页码。保持自包含 —— 除占位文字外不要引入外部 URL。
