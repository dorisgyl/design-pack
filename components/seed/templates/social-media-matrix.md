---
slug: packs/design-pack/templates/social-media-matrix
type: template
lang: en
category: dashboard
title: "NevoFlux Social Matrix Tracker"
title_zh: "社媒矩阵追踪面板"
description: "Cinematic multi-platform analytics dashboard: engagement charts, hover insights, range compare, and a dark/light theme."
tags: [matrix, tracker, multi-platform, template]
sample_image: packs/design-pack/assets/templates/social-media-matrix.svg
source: html-anything/social-media-matrix
---
## Design guidance

A cinematic, data-dense dashboard for tracking one brand across several platforms or channels at once. Intent: a moody, immersive analytics console for a desktop long-scroll page.

Layout:
- Hero header that unifies the brand, a time-window switcher, and a dark/light theme toggle.
- Multi-platform KPI matrix arranged as platforms (across) by metric (down).
- Interactive charts with a hover tooltip and range-compare selection.
- A sticky insights drawer that surfaces text insights about the hovered or pinned data.
- A grid of top posts / top items.

Design details:
- Dark and light themes driven by `data-theme`; both share the same CSS variables, just remapped.
- Cinematic background: layered radial gradients, drifting conic-gradient orbs, a film-grain overlay, and a light streak. No external images.
- Charts are drawn on `<canvas>` (line, stacked bars, donut, geo bars) plus one inline SVG growth curve, so the page stays fully self-contained.
- Charts support hover (details), click (pin a point), drag (interval stats), Shift+Drag (save multiple ranges for an A/B compare), and double-click (reset).
- All numbers come from a small seed dataset in the script, so the page renders with realistic fallback data even before any real feed is wired in.
...
<head>
  ...
  <title>NevoFlux Channel Matrix Dashboard</title>
  <style>
    :root {
      --bg0:#040612;
      --bg1:#070b1a;
...
<body>
  ...
  <div class="app">
    <div class="topbar">
      ...
      <div class="user">
        ...
        <span style="font-size:12px;color:var(--muted)">Workspace</span>
    ...
    <div class="frame">
      ...
      <div class="headline">
        <div>
          <h1 id="title">You shipped 14 Canvas apps this week.</h1>
          <div class="sub" id="subtitle">Activation up 0.4% vs last week. Your top app reached 4.2M agent runs.</div>
        </div>
        <div class="actions">
      ...
      <div class="search">Ask across your workspace - top performing apps last week, GBrain queries from verified members...</div>
      ...
      <div class="grid">
        <div class="card">
          ...
          <div class="legend">
            <span><i class="dot" style="background:var(--yellow)"></i>Pack launch +642</span>
            <span><i class="dot" style="background:var(--pink)"></i>Viral Canvas app +1.2K</span>
            <span><i class="dot" style="background:var(--blue)"></i>Design skill drop +860</span>
          </div>
        </div>

        <div class="card">
          <h3>Top App - This Week</h3>
          <div class="post">
            <div class="head"><span>@nevoflux</span><span id="postPlatform">Browser</span></div>
            <p id="postText">Stopped wiring brittle scripts. Started shipping GBrain-backed Canvas apps. Our throughput 5x'd in a month.</p>
            <div class="thumb"></div>
          </div>
          <div class="bar-list">
            <div class="bar-item"><div class="bar-head"><span>Run success quality</span><span>82%</span></div><div class="track"><span style="width:82%"></span></div></div>
            <div class="bar-item"><div class="bar-head"><span>Member retention</span><span>67%</span></div><div class="track"><span style="width:67%"></span></div></div>
          </div>
        </div>
      </div>
      ...
      <div class="bottom">
        <div class="card list">
          ...
          <div class="row"><span>Canvas app: knowledge frontier explorer</span><span class="pill good">Scheduled</span></div>
          <div class="row"><span>Pack: studio design skills</span><span class="pill">Draft</span></div>
          <div class="row"><span>Essay: why we stopped wiring scripts</span><span class="pill good">Scheduled</span></div>
          <div class="row"><span>Case study: enterprise GBrain rollout</span><span class="pill">Idea</span></div>
        </div>

        <div class="card table">
          <h3>Surface Benchmark</h3>
          <div class="row head"><span>Surface</span><span>Members</span><span>Growth</span><span>Act.</span></div>
          <div class="row"><span>You (NevoFlux)</span><span>610.4K</span><span style="color:var(--good)">+4.2%</span><span>6.8%</span></div>
          <div class="row"><span>GBrain knowledge base</span><span>4.8M</span><span>+1.8%</span><span>5.4%</span></div>
          <div class="row"><span>Canvas apps</span><span>2.2M</span><span>+2.4%</span><span>4.1%</span></div>
        ...
        <div class="card list">
          ...
          <div class="row"><span>Runs</span><span>64%</span></div>
          <div class="row"><span>Edits</span><span>19%</span></div>
          <div class="row"><span>Shares</span><span>12%</span></div>
      ...
      <div class="deep-grid">
        ...
        <div class="card list">
          ...
          <div class="row"><span>Canvas apps</span><span style="color:var(--good)">+19%</span></div>
          <div class="row"><span>Design packs</span><span style="color:var(--good)">+15%</span></div>
          <div class="row"><span>#AgentSDK</span><span style="color:var(--danger)">-4%</span></div>
          <div class="row"><span>Browser sync</span><span style="color:var(--danger)">-8%</span></div>
      ...
      <div class="deep-grid">
        <div class="card">
          <h3>Cross-surface Conversion Funnel</h3>
          <div class="bar-list">
            ...
            <div class="bar-item"><div class="bar-head"><span>Workspace Visits</span><span>428K</span></div><div class="track"><span style="width:41%"></span></div></div>
            <div class="bar-item"><div class="bar-head"><span>Activated</span><span>49K</span></div><div class="track"><span style="width:24%"></span></div></div>
            <div class="bar-item"><div class="bar-head"><span>Paid Seats</span><span>8.2K</span></div><div class="track"><span style="width:11%"></span></div></div>
          </div>
        </div>
        <div class="card">
          <h3>Response SLA</h3>
      ...
      <div class="tri-grid">
        <div class="card">
          <h3>Revenue vs Compute Cost</h3>
          <div class="canvas-wrap" style="height:190px"><canvas id="roiChart" width="420" height="190"></canvas></div>
          <div class="sub">Track profitability trend across agent operations.</div>
        </div>
        <div class="card">
          <h3>Member Cohort Retention</h3>
        ...
        <div class="card">
          ...
          <div class="sub">Region contribution to total activations.</div>
        </div>
      </div>
  ...
  <script>
    const data = {
      x:{title:"You shipped 14 Canvas apps this week.",subtitle:"Activation up 0.4% vs last week. Your top app reached 4.2M agent runs.",growth:"184.3K",growthDelta:"+4.2K net new",post:"Stopped wiring brittle scripts. Started shipping GBrain-backed Canvas apps. Our throughput 5x'd in a month.",postPlatform:"Browser",eng:"4.8%",kpis:[["Members","184.3K","+4.18%"],["Activation Rate","4.8%","+0.4%"],["Runs - 7D","12.8K","+18%"],["Shares - 7D","3.1K","+9%"]],platforms:[["Browser","184.3K members","+2.1K"],["GBrain","42.7K sources","+1.6K"],["Canvas","287K apps","+4.3K"],["Agent SDK","96.4K calls","+1.9K"]]},
      linkedin:{title:"You shipped 6 GBrain sources this week.",subtitle:"Activation up 0.6% vs last week. Your top source reached 980K agent runs.",growth:"42.7K",growthDelta:"+1.6K net new",post:"We indexed an entire wiki into GBrain in a weekend. No pipeline, no crew, one workspace.",postPlatform:"GBrain",eng:"7.2%",kpis:[["Sources","42.7K","+3.7%"],["Activation Rate","7.2%","+0.3%"],["Queries - 7D","6.4K","+10%"],["Shares - 7D","914","+9%"]],platforms:[["Browser","184.3K members","+2.1K"],["GBrain","42.7K sources","+1.6K"],["Canvas","287K apps","+4.3K"],["Agent SDK","96.4K calls","+1.9K"]]},
      youtube:{title:"You shipped 3 Canvas templates this week.",subtitle:"Activation up 0.8% vs last week. Your top template reached 1.8M agent runs.",growth:"287K",growthDelta:"+4.3K net new",post:"We built an internal tool in a weekend using NevoFlux Canvas and one clean GBrain index.",postPlatform:"Canvas",eng:"9.4%",kpis:[["Apps","287K","+1.5%"],["Activation Rate","9.4%","+0.6%"],["Runs - 7D","48.2K","+18%"],["Shares - 7D","2.2K","+9%"]],platforms:[["Browser","184.3K members","+2.1K"],["GBrain","42.7K sources","+1.6K"],["Canvas","287K apps","+4.3K"],["Agent SDK","96.4K calls","+1.9K"]]},
      instagram:{title:"You shipped 9 agent flows this week.",subtitle:"Activation up 0.5% vs last week. Your top flow reached 1.1M agent runs.",growth:"96.4K",growthDelta:"+1.9K net new",post:"Clean SDK, no glue code. Simplicity outperforms over-engineering every time.",postPlatform:"Agent SDK",eng:"6.1%",kpis:[["Calls","96.4K","+2.01%"],["Activation Rate","6.1%","+0.5%"],["Runs - 7D","22.8K","+18%"],["Shares - 7D","1.4K","+9%"]],platforms:[["Browser","184.3K members","+2.1K"],["GBrain","42.7K sources","+1.6K"],["Canvas","287K apps","+4.3K"],["Agent SDK","96.4K calls","+1.9K"]]}
    };
    ...
    function buildHeatmap(){
      ...
      for(let i=0;i<72;i++){
        ...
        cell.title = `Slot ${i+1} · activity ${(42 + (i%13)*3)}%`;
    ...
    function bindChartInteractions(){
      ...
      lineIds.forEach((id)=>{
        ...
        c.addEventListener('mousemove',(e)=>{
          ...
          updateInsights({
            focus: `<b>${nameMap[id]}</b><br/>${st.labels[nearest]} = ${st.values[nearest]}${st.unit}`,
            range: `WoW <b>${mom>=0?'+':''}${mom.toFixed(1)}%</b> · vs baseline <b>${yoy>=0?'+':''}${yoy.toFixed(1)}%</b>`
          });
          showTooltip(`<b>${st.labels[nearest]}</b><br/>Value: ${st.values[nearest]}${st.unit}<br/>WoW: ${mom>=0?'+':''}${mom.toFixed(1)}% · vs baseline: ${yoy>=0?'+':''}${yoy.toFixed(1)}%`, e.clientX, e.clientY);
        });
      ...
      if(mix && !mix.dataset.bound){
        ...
        mix.addEventListener('mousemove',(e)=>{
          ...
          if(hit >= 0){
            ...
            updateInsights({
              focus: `<b>App Mix · ${st.labels[hit]}</b><br/>Canvas ${s[0]} · Agent ${s[1]} · GBrain ${s[2]}`
            });
          } else hideTooltip();
        });
      ...
      if(donut && !donut.dataset.bound){
        ...
        donut.addEventListener('mousemove',(e)=>{
          ...
          if(hit >= 0){
            ...
            updateInsights({
              focus: `<b>Feedback · ${a.label}</b><br/>Share: ${a.value}%`
            });
          } else hideTooltip();
        });
      ...
      if(geo && !geo.dataset.bound){
        ...
        geo.addEventListener('mousemove',(e)=>{
          ...
          if(hit >= 0){
            ...
            showTooltip(`<b>${b.label}</b><br/>Activation share: ${b.value}%`, e.clientX, e.clientY);
            updateInsights({ focus: `<b>Geo Contribution</b><br/>${b.label} contributes ${b.value}% activations` });
          } else hideTooltip();
        });
...
- `topbar / brand` — NevoFlux brand plus an `Analytics` tag; the right side holds the theme switch and the time-window chips (`7D` / `30D` / `90D` / `YTD`).
- `headline` — hero title and subtitle; both are rewritten per surface by the `data` map in the script (`title`, `subtitle`).
- `actions` — header buttons (Export report, Auto-summary, primary `New app +`).
- `insights-panel` — sticky drawer with four slots (`Insight Focus`, `Pinned Point`, `Range Compare`, `A/B Compare`) updated live as you hover, pin, or drag charts.
- `platforms` (`#platforms`) — the surface matrix (Browser, GBrain, Canvas, Agent SDK); rendered from `data[...].platforms` as `[name, value, delta]`.
- `kpis` (`#kpis`) — 4 metric cards rendered from `data[...].kpis` as `[label, value, delta]`.
- `grid` — left card is the 30-day workspace growth curve (inline SVG `path` plus a legend); right card is the top app of the week with two `track` progress bars.
- `bottom` — three cards: Upcoming + Drafts list with status `pill`s, a Surface Benchmark table, and an Activation Rate breakdown.
- Chart canvases — `hourChart` (activity by hour), `mixChart` (stacked app mix), `sentimentChart` (feedback donut), `slaChart` (response SLA), `roiChart` (revenue vs compute cost), `cohortChart` (member cohort retention), `geoChart` (geo share). Edit the arrays in `drawAllCharts()` to reshape any series.
- Interaction model on the line charts: hover for details, click to pin a point, drag for interval stats, Shift+Drag to save ranges for A/B compare, double-click to reset.
- The seed `data` object drives the auto-rotating header; replace its values to wire in real numbers while keeping the layout intact.
```
```
