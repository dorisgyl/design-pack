---
slug: packs/design-pack/templates/meeting-notes
type: template
lang: en
category: doc
title: "Meeting Notes"
title_zh: "会议纪要"
description: "A long-form meeting minutes layout: title, attendees, agenda, decisions, action items, and next meeting."
tags: [minutes, meeting, "1:1", 纪要, template]
...
A modern meeting-minutes document with a strong emphasis on action items. It reads as a long, scrollable page on letter-width paper.

Layout, top to bottom:

- **Title bar** — meeting name plus the date/time/location/note-taker meta row, and a row of attendee avatars (with an "apologies" note for anyone absent).
- **Agenda checklist** — each item is a row with a checkbox (filled when done), a title, a one-line description, and a time slot.
- **Decisions block** — a soft, rounded card with a colored left border that records what was agreed, on the record.
- **Action items table** — columns for Action, Owner (avatar + name), Due, and Status, with colored status pills (To do / In progress / Blocked / Done).
- **Open questions + next meeting** — a two-column grid of bordered panels.
- **Footer** — a monospace strip with the document version and where it was filed.

Design details: serif display headings against a sans body, a muted monospace for labels and timestamps, generous whitespace, and a single accent color used for decisions and in-progress states. Avatars are gradient circles seeded from initials. The page collapses the two-column grid to a single column on narrow screens.
...
<title>NevoFlux core weekly · 14 Oct notes</title>
<style>
  :root {
    --bg: #fafaf8;
    --paper: #ffffff;
...
<div class="page">
  <header class="head">
    <div class="crumb">NevoFlux / Core squad / Weeklies</div>
    <h1>NevoFlux core weekly · W42</h1>
    <div class="meta-row">
      <span><strong>Date</strong>Tuesday, 14 October 2025</span>
      <span><strong>Time</strong>10:00 – 11:00 PT</span>
      <span><strong>Where</strong>NevoFlux Canvas room · core-weekly</span>
  ...
  <section>
    ...
    <div class="agenda">
      ...
      <div class="agenda-item done">
        <div class="check">✓</div>
        <div class="body"><strong>GBrain ingest workstream — M2 risk</strong><small>Embedding-pipeline microcopy review is the open dependency.</small></div>
        <div class="time">10:15 · 10m</div>
      </div>
      <div class="agenda-item done">
        <div class="check">✓</div>
        <div class="body"><strong>Canvas activation metrics review</strong><small>First-app rate up 9 pp WoW; debrief the empty-canvas work.</small></div>
        <div class="time">10:25 · 10m</div>
      </div>
      <div class="agenda-item done">
        <div class="check">✓</div>
        <div class="body"><strong>Pioneer agent-SDK review prep</strong><small>Partner loop-in for Thursday's call.</small></div>
      ...
      <div class="agenda-item">
        <div class="check"></div>
        <div class="body"><strong>Open thread — anything else</strong><small>Pushed to async — see #core-squad.</small></div>
  ...
  <section>
    ...
    <div class="decisions">
      ...
      <ul>
        <li><strong>M2 (GBrain re-index step)</strong> stays at Nov 18 unless the pipeline review slips past Wednesday EOD; Devon owns the escalation.</li>
        <li><strong>Empty-canvas experiment</strong> rolls to 100% on Thursday after one more 24h hold; no follow-up control needed.</li>
        <li><strong>Q4 themes</strong>: (1) Enterprise-ready browser (SSO + audit), (2) Canvas onboarding 2.0, (3) Pack marketplace for design skills. Sasha to write up the one-pagers.</li>
  ...
  <section>
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          <td>Pack-build backlog dashboard</td>
          <td class="owner"><span class="av a-ca">CA</span>Caleb</td>
          <td>Tue Oct 21</td>
          <td><span class="pill pill-blocked">Blocked · awaiting metrics ACL</span></td>
        </tr>
        <tr>
  ...
  <section>
    ...
    <div class="grid">
      <div class="panel">
        ...
        <p>Do we want a customer in the Q4 pack-marketplace kickoff (Pioneer would say yes), or do we keep the first session internal?</p>
      ...
      <div class="panel">
        ...
        <div class="next-meeting"><strong>Time</strong>10:00 – 11:00 PT · Canvas room</div>
  ...
  <footer>
    <span>NevoFlux Core squad · Notes v1</span>
    <span>Filed in #core-squad · 15 Oct 2025</span>
  </footer>
</div>
</body>
</html>
...
Fill these sections / slots:

- **Header** — breadcrumb (org / team / cadence), meeting title, and the meta row: Date, Time, Where, Notes by. Replace the attendee avatars (initials + `a-*` gradient class) and the "apologies" line for anyone absent.
- **Agenda** — one `.agenda-item` per topic; add the `done` class to fill its checkbox. Each item has a title (`strong`), a one-line description (`small`), and a time slot.
- **Decisions** — list of agreed outcomes; lead each with a bold subject.
- **Action items** — one table row per task: Action, Owner (avatar + name), Due, and a status pill (`pill-todo`, `pill-progress`, `pill-blocked`, `pill-done`).
- **Open questions & next meeting** — left panel for unresolved questions, right panel for the next session's Date / Time / Pre-read / Notes by.
- **Footer** — document version and where the notes were filed.
